...
//	new /obj/item/weapon/tgpickaxe(src)
	new /obj/item/clothing/glasses/meson(src)


/**********************Shuttle Computer**************************/

var/mining_shuttle_tickstomove = 5
...
/obj/machinery/computer/mining_shuttle/attack_hand(user as mob)
	...
	var/dat
	var/datloc = null
	dat = text("<b>Mining shuttle:</b> <A href='?src=\ref[src];move=[1]'>Move</A><br>Shuttle location: \green [datloc]")
	user << browse("[dat]", "window=miningshuttle;size=200x100")

	if (location == 0)
		datloc = "SS13"
	else if (location == 1)
		datloc = "Mining Outpost"
...
/obj/machinery/computer/mining_shuttle/Topic(href, href_list)
	...
	if(href_list["move"])
		if (!mining_shuttle_moving)
			for(var/mob/M in viewers(4, get_turf(usr.machine)) ) //gotta rework this later --soyuz
				M << "\red The Mining Shuttle has been called and will move shortly."
			move_mining_shuttle()
			sleep mining_shuttle_tickstomove*10
			for(var/mob/M in viewers(4, get_turf(usr.machine)) )
				M << "\red The Mining Shuttle has moved!"
		else
			usr << "\blue The shuttle is already moving!"
...
/obj/machinery/computer/mining_shuttle/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/weapon/card/emag))
		src.req_access = list()
		hacked = 1
		usr << "The computer's IDcheck has been disabled."
