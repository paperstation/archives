...
/obj/item/device/log
	desc = "A device that records the logs of station personnel."
	name = "tape recorder"
	icon_state = "taperecorderidle"
	item_state = "analyzer"
	w_class = 1.0
...
/obj/item/device/log/hear_talk(mob/M as mob, msg)
	if (src.recording)
		var/ending = copytext(msg, length(msg))
		if (M.stuttering)
			src.storedinfo += "[M.name] stammers, \"[msg]\""
...
/obj/item/device/log/attackby(obj/item/weapon/W as obj, mob/user as mob)
	..()
	if (istype(W, /obj/item/weapon/card/emag))
		if (src.emagged == 0)
			src.emagged = 1
...
/obj/item/device/log/proc/explode()
	var/turf/T = get_turf(src.loc)
	if (ismob(src.loc))
		var/mob/M = src.loc
		M.show_message("\red The [src] explodes!", 1)
...
/obj/item/device/log/verb/record()
	set name = "Start Recording"
	set category = "Object"
	if(usr.stat)
		usr << "Not when you're incapicated."
...
/obj/item/device/log/verb/stop_recording()
	set name = "Stop Recording"
	set category = "Object"
	if(usr.stat)
		usr << "Not when you're incapicated."
...
/obj/item/device/log/verb/clear_memory()
	set name = "Clear Memory"
	set category = "Object"
	if(usr.stat)
		usr << "Not when you're incapicated."
...
/obj/item/device/log/verb/playback_memory()
	set name = "Playback Memory"
	set category = "Object"
	if(usr.stat)
		usr << "Not when you're incapicated."
