	...
	if (src.shots > 0)
		...
		if (istype(M, /mob/living/carbon/human))
			...
			if (istype(H.head, /obj/item/clothing/head/helmet/welding))
				if(!H.head:up)
					safety = 1
		if (istype(M, /mob/living/carbon/alien))//So aliens don't get flashed (they have no external eyes)
			safety = 1
		if(isrobot(user))
			spawn(0)
				...
				del(animation)
		if (!( safety ))
			if (M.client)//Probably here to prevent forced conversion of dced players.
				if (status == 0)
