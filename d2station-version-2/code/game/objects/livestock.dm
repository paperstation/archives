...
/obj/livestock
	...
	examine()
		...
		..()
		if(!alive)
			usr << text("\red <B>The [src] is not moving</B>")
		else if (src.health == src.health)
			usr << text("\red <B>The [src] looks healthy.</B>")
		else
			usr << text("\red <B>The [src] looks beat up.</B>")
		if (aggressive && alive)
...
//RANDOM LIVESTOCK SHIT BY ERIKA
...
/obj/livestock/zombie
	name = "Zombie"
	desc = "Looks like it used to be human."
	icon = 'mob.dmi'
	icon_state = "zombie"
	aggressive = 1
	health = 35
	...
	special_attack()
		if (prob(bite_chance))
			target.show_message("\red <B>[src] has bit you!</B>")
