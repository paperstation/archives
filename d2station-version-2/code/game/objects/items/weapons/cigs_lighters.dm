...
//CIGARETTES//
//////////////
/obj/item/weapon/reagent_containers/cigs/attackby(obj/item/weapon/W as obj, mob/user as mob)
	..()
	...
	if(istype(W, /obj/item/device/igniter))
		if(src.lit == 0)
			...
			src.item_state = "cigon"
			for(var/mob/O in viewers(user, null))
				O.show_message(text("\red [] plainly lights the cigarette with []. How exciting.", user, W), 1)
			spawn() //start fires while it's lit
				src.process()


/obj/item/weapon/reagent_containers/cigs/process()
...
/obj/item/weapon/reagent_containers/cigs/dropped(mob/user as mob)
	if(src.lit == 1)
		for(var/mob/O in viewers(user, null))
...
/obj/item/weapon/cigpacket/attack_hand(mob/user as mob)
	if(user.r_hand == src || user.l_hand == src)
		if(src.cigcount == 0)
			...
			src.cigcount--
			var/obj/item/weapon/reagent_containers/cigs/cigarette/W = new /obj/item/weapon/reagent_containers/cigs/cigarette(user)
			if(user.hand)
				user.l_hand = W
			else
				user.r_hand = W
