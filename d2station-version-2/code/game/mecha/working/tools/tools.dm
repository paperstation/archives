		...
		spawn(tool_cooldown)
			if(chassis)
				if(T == chassis.loc && src == chassis.selected_tool)
					...
					else if(istype(target, /turf/simulated/asteroid/wall))
						var/turf/simulated/asteroid/wall/M = target
...
/datum/mecha_tool/extinguisher
	...
	action(atom/target) //copypasted from extinguisher. TODO: Rewrite from scratch.
		...
		spawn(tool_cooldown)
			tool_ready = 1
		if(istype(target, /obj/reagent_dispensers/foamtank) && get_dist(chassis,target) <= 1)
			var/obj/o = target
			...
			if(src.reagents.total_volume > 0)
				playsound(chassis, 'extinguish.ogg', 40, 1, -3)
