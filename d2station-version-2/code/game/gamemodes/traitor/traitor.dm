...
/datum/game_mode/traitor/pre_setup()
	...
	if(traitor_scaling)
		...
		num_traitors = max(1, min(round(num_players) + 1), traitors_possible)
...
/datum/game_mode/traitor/post_setup()
	for(var/datum/mind/traitor in traitors)
		...
		spawn(rand(10,100))
			traitor.current << "<B>You are a traitor.</B>"
			traitor.current << "You were not given any special items, we expect you to use items on the station to complete your objectives."
			update_traitor_icons(traitor.current)
			var/obj_count = 1
...
/datum/game_mode/traitor/proc/update_traitor_icons()
	spawn(0)
		return
		/*for(var/datum/mind/traitor in ticker.mode.traitors)
			if(traitor.current)
				if(traitor.current.client)
					var/I = image('mob.dmi', loc = traitor.current, icon_state = "traitor")
					traitor.current.client.images += I*/
...
/datum/game_mode/traitor/send_intercept()
	...
	possible_modes.Add("wizard", "nuke", "traitor", "malf", "changeling")
