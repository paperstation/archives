...
/obj/machinery/computer/enginecontrol/process()
	for(var/obj/machinery/power/generator_type2/M in machines)
		totalp = round(M.lastgen *6)
		Efficiency = round(100 / 155000 * M.lastgen)
		src.updateDialog()
...
/obj/machinery/computer/enginecontrol/proc/interact(mob/user)
	...
	if(!(totalp))
		dat += "<B><BR><font color=\"darkred\">Total Power: 0 watts</B></font><BR>"
		dat += "<B><font color=\"darkred\">Engine Efficiency: at 0%</B></font><BR>"
	if(totalp)
		dat += "<B><BR>Total Power: [round(totalp, 0.1)] watts</B></font><BR>"
	...
	user << browse(dat, "window=computer;size=330x400")
