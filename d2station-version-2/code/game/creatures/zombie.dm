#define cycle_pause 1 //min 1
#define viewrange 15 //min 2
...
/obj/creature/zombie
	name = "zombie"
	desc = "Looks almost like it used to be human."
	icon = 'mob.dmi'
	icon_state = "zombie"
	layer = 5.0
	density = 1
	...
	examine()
		...
		..()
		if(!alive)
			usr << text("\red <B>It doesn't look animated.</B>")
		else if (src.health > 10)
			usr << text("\red <B>A zombie.</B>")
		else
			usr << text("\red <B>A zombie. It looks like it's about to fall apart.</B>")
		return
	...
	bullet_act(flag, A as obj)
		if (flag == PROJECTILE_BULLET)
			...
			src.health -= 8
		else if (flag == PROJECTILE_MAGNET)
			src.health -= 60
		healthcheck()
				...
				if(distance <= 1)
					if(prob(30))
						for(var/mob/O in viewers(world.view,src))
							O.show_message("\red <B>[src] flails at [src.target]!</B>", 1, "\red You hear the sounds of combat", 2)
						target:bruteloss += rand(10,40)
						sleep(5)
						if(prob(10))
							target:paralysis = max(target:paralysis, 2)
					if(prob(10))
						O.show_message("\red <B>[src] bites [src.target]!</B>", 1, "\red You hear the sounds of biting", 2)
						target:contract_disease(new /datum/disease/tvirus,1)
	...
	proc/path_idle(var/atom/trg)
		...
		if(prob(10))
			for(var/mob/O in viewers(world.view,src))
				O.show_message("A zombie moans.", 1, "You hear what seems like a faint moaning noise", 2)
	...
	proc/death()
		...
		density = 0
		icon_state = "zombie_l"
		new /obj/item/weapon/reagent_containers/food/snacks/humanmeat
		new /obj/item/weapon/reagent_containers/food/snacks/humanmeat
		new /obj/decal/cleanable/blood/gibs/body
		new /obj/decal/cleanable/blood/gibs/limb
		new /obj/decal/cleanable/blood/gibs/limb
		new /obj/decal/cleanable/blood/gibs/core
		set_null()
		for(var/mob/O in hearers(src, null))
			O.show_message("\red <B>[src] falls apart!</B>", 1)
		del(src)
