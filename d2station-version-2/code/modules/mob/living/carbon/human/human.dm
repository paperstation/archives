			...
			if(src.loc:sd_lumcount < 3 || src.blinded)
				...
				if (world.time > src.lastDblClick+2)
					...
					if((prob(40)) || (prob(95) && src.mutations & 16))
						...
						src.visible_message("\red [src] accidentally stun \themself with the [W.name].", \
				...
				if (prob(90))
					...
					if (prob(70))
						...
						if (prob(60))
							d = d / 2
						d = d / 5
		if (istype(src.wear_suit, /obj/item/clothing/suit/armor/spessmarine))
			if (prob(99))
				show_message("\red Your armor absorbs the blow!", 4)
				return
			else
...
/*		if("headset")
		...
		if("ears")
			...
			src.u_equip(W)
			src.ears = W
			if(istype(W,/obj/item/clothing/head/kittyears))
				W.update_icon(src)
			W.equipped(src, text)
		if("i_clothing")
			if (src.w_uniform)
...
/*	if (src.w_radio)
	...
	if (src.wear_suit)
		...
		if (istype(src.wear_suit, /obj/item/clothing/suit/straight_jacket))
			...
			if ((src.l_hand || src.r_hand))
				...
				drop_item()
				src.hand = h
		if ((istype(src.wear_suit, /obj/item/clothing/suit/armor/rig)) && (src.healths) && (!src.lying))
			if (src.stat != 2)
	...
	if (src.head)
		...
		if(istype(src.head,/obj/item/clothing/head/kittyears))
	...
	if (cloaked)
		src.overlays += image("icon" = 'mob.dmi', "icon_state" = "cloak", "layer" = MOB_LAYER)
		src.invisibility = 2
	else
		src.invisibility = 0
...
/mob/living/carbon/human/attack_paw(mob/M as mob)
	...
	if (M.a_intent == "help")
		...
		for(var/mob/O in viewers(src, null))
			O.show_message("\blue [M.name] shakes [src.name] trying to wake them up!", 1)
		/*else
			for(var/mob/O in viewers(src, null))
...
/mob/living/carbon/human/proc/take_organ_damage(var/brute, var/burn)
	var/list/parts = list()
