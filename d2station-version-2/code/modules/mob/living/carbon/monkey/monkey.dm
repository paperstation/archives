...
/mob/living/carbon/monkey/rat/New()
	spawn(1)
		var/datum/reagents/R = new/datum/reagents(1000)
		reagents = R
		R.my_atom = src
		if (!(src.dna))
			...
			if (src.gender == "male")
				...
				gendervar = add_zero2(num2hex((rand(2051,4094)),1), 3)
			src.dna.uni_identity += gendervar
			src.dna.uni_identity += "R47"
			src.dna.uni_identity += "M05"
			src.health = "5"

		if(src.name == "rat") src.name = text("rat ([rand(1, 1000)])")
...
/mob/living/carbon/monkey/rat/Bump(atom/movable/AM as mob|obj, yes)
	return
...
/mob/living/carbon/monkey/bullet_act(flag)
	...
	if (flag == PROJECTILE_BULLET)
		...
		if (src.stat != 2)
			src.bruteloss += 100
			src.health = 10 - src.oxyloss - src.toxloss - src.fireloss - src.bruteloss
			if (prob(25))
				src.stunned = 1
...
/mob/living/carbon/monkey/update_clothing()
	...
	if(src.update_icon) // Skie
		...
		if(istype(src, /mob/living/carbon/monkey/elf))
			if (!( src.lying ))
				src.icon_state = "monkey1xmas"
			else
				src.icon_state = "monkey0xmas"
		else
			if (!( src.lying ))
...
/mob/living/carbon/monkey/rat/brown/update_clothing()
	if(src.buckled)
		if(istype(src.buckled, /obj/stool/bed))
			src.lying = 1
		else
	...
	if(src.update_icon) // Skie
		...
		if (!( src.lying ))
			src.icon_state = "brownrat1"
		else
			src.icon_state = "brownrat0"
	return

/mob/living/carbon/monkey/rat/white/update_clothing()
	if(src.buckled)
		if(istype(src.buckled, /obj/stool/bed))
			src.lying = 1
		else
	...
	if(src.update_icon) // Skie
		...
		if (!( src.lying ))
			src.icon_state = "whiterat1"
		else
			src.icon_state = "whiterat0"
	return

mob/living/carbon/monkey/mellens/update_clothing()
	if(src.buckled)
		if(istype(src.buckled, /obj/stool/bed))
			src.lying = 1
		else
	...
	if(src.update_icon) // Skie
		...
		if (!( src.lying ))
			src.icon_state = "melons1"
		else
			src.icon_state = "melons0"
	return


mob/living/carbon/monkey/muggles/update_clothing()
	if(src.buckled)
		if(istype(src.buckled, /obj/stool/bed))
			src.lying = 1
		else
	...
	if(src.update_icon) // Skie
		...
		if (!( src.lying ))
			src.icon_state = "muggles1"
		else
			src.icon_state = "muggles0"
	return

mob/living/carbon/monkey/mrsmuggles/update_clothing()
	if(src.buckled)
		if(istype(src.buckled, /obj/stool/bed))
			src.lying = 1
		else
	...
	if(src.update_icon) // Skie
		...
		if (!( src.lying ))
			src.icon_state = "mrsmuggles1"
		else
			src.icon_state = "mrsmuggles0"
	return


mob/living/carbon/monkey/rathen/update_clothing()
	if(src.buckled)
		if(istype(src.buckled, /obj/stool/bed))
			src.lying = 1
		else
	...
	if(src.update_icon) // Skie
		...
		if (!( src.lying ))
			src.icon_state = "rathen1"
		else
			src.icon_state = "rathen0"
	return
