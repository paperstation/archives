...
/mob/dead/observer/proc/respawn()
	if(!resptim)
		resptim = world.time
	...
	if ((resptime == 0) && (ticker.mode.name == "extended"))
		usr << "You may now respawn if you wish!"
		if(alert("Do you wish to respawn?",,"Yes","No")=="Yes")
			usr << "\blue <B>Please play correctly and do not use the same name you died with, and do not metagame, it is against the rules!</B>"
			respcounter += 1
			if(respcounter > 0)
				resptime += 20
			if(!src.client)
			...
			resptime = 10
			src.achievement_give("Temptation!", 11, 70)
	spawn(20)
		respawn()
...
/mob/dead/observer/Stat()
	...
	if((ticker.current_state == GAME_STATE_PLAYING) && (ticker.mode.name == "extended"))
