...
/turf/simulated/asteroid/wall //wall piece
	name = "Asteroid"
	icon = 'walls.dmi'
	icon_state = "asteroid"
	oxygen = 0
	...
	var/mineralName = ""
	var/mineralAmt = 0
	var/spread = 0 //will the vein spread?
	var/spreadChance = 0 //the percentual chance of an ore spreading to the neighbouring tiles

/turf/simulated/asteroid/wall/Del()
	return
...
/turf/simulated/asteroid/wall/New()
	if (mineralName && mineralAmt && spread && spreadChance)
		if(prob(spreadChance))
			if(istype(get_step(src, SOUTH), /turf/simulated/asteroid/wall/random))
				new src.type(get_step(src, SOUTH))
		if(prob(spreadChance))
			if(istype(get_step(src, NORTH), /turf/simulated/asteroid/wall/random))
				new src.type(get_step(src, NORTH))
		if(prob(spreadChance))
			if(istype(get_step(src, WEST), /turf/simulated/asteroid/wall/random))
				new src.type(get_step(src, WEST))
		if(prob(spreadChance))
			if(istype(get_step(src, EAST), /turf/simulated/asteroid/wall/random))
				new src.type(get_step(src, EAST))
	return

/turf/simulated/asteroid/wall/random
...
/turf/simulated/asteroid/wall/random/New()
	if (prob(mineralChance))
		var/mName = pickweight(mineralSpawnChanceList) //temp mineral name
		//spawn(5)
		if (mName)
			var/turf/simulated/asteroid/wall/M
			switch(mName)
				if("Uranium")
					M = new/turf/simulated/asteroid/wall/uranium(src)
				if("Iron")
					M = new/turf/simulated/asteroid/wall/iron(src)
				if("Diamond")
					M = new/turf/simulated/asteroid/wall/diamond(src)
				if("Gold")
					M = new/turf/simulated/asteroid/wall/gold(src)
				if("Silver")
					M = new/turf/simulated/asteroid/wall/silver(src)
				if("Plasma")
					M = new/turf/simulated/asteroid/wall/plasma(src)
			if(M)
				src = M
				M.levelupdate()
	return

/turf/simulated/asteroid/wall/random/Del()
	return

/turf/simulated/asteroid/wall/uranium
	name = "Asteroid"
	icon_state = "asteroid_uranium"
	mineralName = "Uranium"
	mineralAmt = 5
	spreadChance = 10
	spread = 1

/turf/simulated/asteroid/wall/iron
	name = "Asteroid"
	icon_state = "asteroid_iron"
	mineralName = "Iron"
	mineralAmt = 5
	spreadChance = 25
	spread = 1

/turf/simulated/asteroid/wall/diamond
	name = "Asteroid"
	icon_state = "asteroid_diamond"
	mineralName = "Diamond"
	mineralAmt = 5
	spreadChance = 10
	spread = 1

/turf/simulated/asteroid/wall/gold
	name = "Asteroid"
	icon_state = "asteroid_gold"
	mineralName = "Gold"
	mineralAmt = 5
	spreadChance = 10
	spread = 1

/turf/simulated/asteroid/wall/silver
	name = "Asteroid"
	icon_state = "asteroid_silver"
	mineralName = "Silver"
	mineralAmt = 5
	spreadChance = 10
	spread = 1

/turf/simulated/asteroid/wall/plasma
	name = "Asteroid"
	icon_state = "asteroid_plasma"
	mineralName = "Plasma"
	mineralAmt = 5
	spreadChance = 25
	spread = 1

/turf/simulated/asteroid/wall/clown
	name = "Asteroid"
	icon_state = "asteroid_clown"
	mineralName = "Clown"
	mineralAmt = 3
	spreadChance = 0
	spread = 0

/turf/simulated/asteroid/wall/proc/ReplaceWithAsteroidFloor()
...
/*	if (istype(W, /obj/item/weapon/pickaxe/radius))
		...
		user << "\red You start picking."//YOUR NOSE
...
/turf/simulated/asteroid/wall/proc/gets_drilled()
...
/*
/turf/simulated/asteroid/wall/proc/setRandomMinerals()
	var/s = pickweight(list("uranium" = 5, "iron" = 50, "gold" = 5, "silver" = 5, "plasma" = 50, "diamond" = 1))
	if (s)
		mineralName = s

	var/N = text2path("/turf/simulated/asteroid/wall/[s]")
	if (N)
		var/turf/simulated/asteroid/wall/M = new N
		src = M
		if (src.mineralName)
			mineralAmt = 5
	return*/
...
/turf/simulated/floor/airless/asteroid/proc/updateMineralOverlays()
	...
	if(istype(get_step(src, NORTH), /turf/simulated/asteroid/wall))
		src.overlays += image('walls.dmi', "rock_side_n")
	if(istype(get_step(src, SOUTH), /turf/simulated/asteroid/wall))
		src.overlays += image('walls.dmi', "rock_side_s", layer=6)
	if(istype(get_step(src, EAST), /turf/simulated/asteroid/wall))
		src.overlays += image('walls.dmi', "rock_side_e", layer=6)
	if(istype(get_step(src, WEST), /turf/simulated/asteroid/wall))
...
/obj/ore_box/attack_hand(obj, mob/user as mob)
	...
	for (var/obj/item/weapon/ore/C in contents)
		...
		if (istype(C,/obj/item/weapon/ore/clown))
			amt_clown++;
		if (istype(C,/obj/item/weapon/ore/rock))
			amt_rock++;
		if (istype(C,/obj/item/weapon/ore/coal))
			amt_coal++;
		if (istype(C,/obj/item/weapon/ore/slag))
			amt_slag++;
	...
	if (amt_rock)
		dat += text("Rock: [amt_rock]<br>")
	if (amt_coal)
		dat += text("Coal ore: [amt_coal]<br>")
	if (amt_slag)
		dat += text("Slag: [amt_slag]<br>")
		...
		switch(href_list["release"])
			...
			if ("glass")
				if (machine.ore_glass > 0)
					...
					machine.ore_glass = 0
			if ("rglass")
				if ((machine.ore_rglass > 0) && (machine.ore_iron > 0))
...
/obj/item/weapon/pickaxe/radius // For the side project to make radius cutting devices
	...
	jackhammer
		...
		digspeed = 30
		desc = "You've dug what you dig. Look out for giant lizards."
