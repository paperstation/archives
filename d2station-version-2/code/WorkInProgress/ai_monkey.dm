			...
			if(ai_canspecial)
				...
				if( (distance == 3) && ai_validpath())
					if(valid)
						...
						else if(istype(target.r_hand, /obj/item/weapon/reagent_containers/food/snacks/banana) || istype(target.l_hand, /obj/item/weapon/reagent_containers/food/snacks/banana))
							var/obj/item/weapon/reagent_containers/food/snacks/banana/banananana
							if(istype(target.r_hand, /obj/item/weapon/reagent_containers/food/snacks/banana)) banananana = target.r_hand
							else if(istype(target.l_hand, /obj/item/weapon/reagent_containers/food/snacks/banana)) banananana = target.l_hand
							spawn(0)
								step_towards(src,ai_target)
								step_towards(src,ai_target)
...
/proc/toggleai(var/mob/M in world)
	if(!M.ai_active)
		M:ai_init()
	else
		M.ai_active = 0
