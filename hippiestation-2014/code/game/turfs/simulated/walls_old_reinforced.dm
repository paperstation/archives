...
/turf/simulated/wall/old_wall/attackby(obj/item/W as obj, mob/user as mob)
	...
	switch(d_state)
		...
		if(1)
			if (istype(W, /obj/item/weapon/screwdriver))
				...
				sleep(40)
				if( !istype(src, /turf/simulated/wall/old_wall) || !user || !W || !T )	return

				if( d_state == 1 && user.loc == T && user.get_active_hand() == W )
					src.d_state = 2
					src.icon_state = "oldwall-2"
		...
		if(2)
			if( istype(W, /obj/item/weapon/weldingtool) )
				...
				if( WT.remove_fuel(0,user) )
					...
					sleep(50)
					if( !istype(src, /turf/simulated/wall/old_wall) || !user || !WT || !WT.isOn() || !T )	return

					if( d_state == 2 && user.loc == T && user.get_active_hand() == WT )
						src.d_state = 3
						src.icon_state = "oldwall-3"
