...
obj/machinery/atmospherics/binary/volume_pump
	...
	update_icon()
		if(stat & NOPOWER)
			...
			icon_state = "vpump_intact_[on?("on"):("off")]"
		else
			if(node1)
				icon_state = "vpump_exposed_1_off"
			else if(node2)
				icon_state = "vpump_exposed_2_off"
			else
				icon_state = "vpump_exposed_3_off"
		return
