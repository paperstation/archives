/obj/machinery/atmospherics/unary/vent_scrubber
	...
	update_icon()
		if(node && on && !(stat & (NOPOWER|BROKEN)))
			if(scrubbing)
				icon_state = "[level == 1 && istype(loc, /turf/simulated) ? "h" : "" ]srubber_on"
			else
				icon_state = "[level == 1 && istype(loc, /turf/simulated) ? "h" : "" ]srubber_in"
		else
			icon_state = "[level == 1 && istype(loc, /turf/simulated) ? "h" : "" ]srubber_off"
		return
	...
	process()
		...
		if(stat & (NOPOWER|BROKEN))
			return
		if(kill_air)//if air is off then prevent atmos from doing things
			return 0
		if (!node)
			on = 0
		//broadcast_status()
