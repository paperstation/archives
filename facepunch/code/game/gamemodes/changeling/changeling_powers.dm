	...
	for(var/stage = 1, stage<=3, stage++)
		switch(stage)
			...
			if(3)
				...
				T.deal_damage(40, BRUTE, PIERCE, "chest")
...
/mob/proc/changeling_lesser_form()
	...
	O.deal_overall_damage(C.get_brute_loss(), C.get_fire_loss())
	O.tox_damage = C.tox_damage
	O.oxy_damage = C.oxy_damage
	O.stat = C.stat
	O.a_intent = "hurt"
...
/mob/proc/changeling_fakedeath()
	...
	spawn(rand(800,2000))
		if(changeling_power(20,1,100,DEAD))
			...
			C.stat = CONSCIOUS
			C.tod = null
			C.deal_overall_damage(-1000, -1000)
			C.tox_damage = 0
			C.oxy_damage = 0
			C.deal_damage(-100, WEAKEN)
			C.deal_damage(-100, PARALYZE)
...
/mob/proc/changeling_unstun()
	...
	C.stat = 0
	C.deal_damage(-50, PARALYZE)
	C.deal_damage(-50, WEAKEN)
	C.lying = 0
	C.update_canmove()
...
/mob/proc/changeling_rapidregen()
	...
	spawn(0)
		for(var/i = 0, i<10,i++)
			if(C)
				C.deal_overall_damage(-10, -10)
				C.deal_damage(-10, TOX)
				C.deal_damage(-10, OXY)
				sleep(10)
...
/mob/proc/changeling_paralysis_sting()
	...
	T.deal_damage(20, WEAKEN)
	T.deal_damage(20, STUTTER)
...
/mob/proc/changeling_DEATHsting()
	...
	T.silent = 10
	T.deal_damage(10, PARALYZE)
	T.deal_damage(10, STUTTER)
	T.make_jittery(1000)
