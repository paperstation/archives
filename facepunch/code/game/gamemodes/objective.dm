...
/datum/objective/mutiny
	...
	check_completion()
		if(target && target.current)
			...
			if((T && (T.z != 1)) || !target.current.client)			//If they leave the station or logout they count as dead for this
				return 2
			return 0
		return 1
...
/datum/objective/steal
	...
	var/global/possible_items[] = list(
		...
		"a laser gun" = /obj/item/weapon/gun/energy/laser,
	)
			...
			if("an AI")
				for(var/obj/item/device/aicard/C in all_items) //Check for ai card
					for(var/mob/living/silicon/ai/M in C)
						if(istype(M, /mob/living/silicon/ai)) //AI no longer needs to be alive
							return 1
			else
...
/datum/objective/soul
	proc/gen_amount_goal()
		target_amount = rand(1,4)
		explanation_text = "Create [target_amount] skeletal minions."
		return target_amount
	...
	check_completion()
		if(!ishuman(owner.current))
			return 0
		if(minions >= target_amount)
			return 1
		return 0


/datum/objective/sword
	proc/gen_amount_goal()
		target_amount = rand(1,4)
		explanation_text = "Fully complete your sword."
		return target_amount
