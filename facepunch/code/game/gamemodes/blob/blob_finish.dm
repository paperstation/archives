...
/datum/game_mode/blob/declare_completion()
	...
	if(stage >= 3)
		...
		text += "<br><br><FONT size = 3><B>The blob has taken over the station!</B></FONT>"
		text += "<br><B>The entire station was eaten by the Blob</B>"
		check_quarantine()

	else if(station_was_nuked)
		feedback_set_details("round_end_result","halfwin - nuke")
		text +=  "<br><br><FONT size = 3><B>Partial Win: The station has been destroyed!</B></FONT>"
		text +=  "<br><B>Directive 7-12 has been successfully carried out preventing the Blob from spreading.</B>"

	else
		feedback_set_details("round_end_result","win - blob eliminated")
		text +=  "<br><br><FONT size = 3><B>The staff has won!</B></FONT>"
		text +=  "<br><B>The alien organism has been eradicated from the station</B>"

		var/datum/station_state/end_state = new /datum/station_state()
		end_state.count()
		var/percent = round( 100.0 *  start_state.score(end_state), 0.1)
		text +=  "<br><B>The station is [percent]% intact.</B>"
		log_game("Blob mode was won with station [percent]% intact.")
		text +=  "<br>\blue Rebooting in 30s"
	..()
	outputstats += text
	return 1
