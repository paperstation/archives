...
/datum/game_mode/revolution
	...
	uplink_special_items = 0

	uplink_welcome = "Revolutionary Uplink Console:"
	uplink_uses = 2
	uplink_items = {"Revolutionary Propaganda;/obj/item/device/flash:1:Flash"}
...
/datum/game_mode/revolution/post_setup()
	...
	for(var/datum/mind/rev_mind in head_revolutionaries)
		...
		equip_traitor(rev_mind.current, 1)
...
/datum/game_mode/proc/greet_revolutionary(var/datum/mind/rev_mind, var/you_are=1)
	...
	for(var/datum/objective/objective in rev_mind.objectives)
		...
		obj_count++
	rev_mind.current << "You can order flashes from your PDA uplink."

/////////////////////////////////////////////////////////////////////////////////
//This are equips the rev heads with their gear, and makes the clown not clumsy//
/////////////////////////////////////////////////////////////////////////////////
/datum/game_mode/proc/equip_revolutionary(mob/living/carbon/human/mob)//Currently not used due to revs having PDAs again
	if(!istype(mob))
		return
...
/datum/game_mode/proc/add_revolutionary(datum/mind/rev_mind)
	...
	if((rev_mind in revolutionaries) || (rev_mind in head_revolutionaries))
		return 0
	if(rev_mind.has_been_rev)//You can only be rev'd once
		return 0
	rev_mind.has_been_rev = 1
...
/datum/game_mode/revolution/proc/check_heads_victory()
	for(var/datum/mind/rev_mind in head_revolutionaries)
		...
		if((rev_mind) && (rev_mind.current) && (rev_mind.current.stat != 2) && T && (T.z == 1))
			if(rev_mind.current.client && ishuman(rev_mind.current))//You now need a client to pass the check
				return 0
	return 1
...
/datum/game_mode/revolution/declare_completion()
	...
	if(finished == 1)
		...
		text += "\red <br><FONT size = 3><B> The heads of staff were killed or abandoned the station! The revolutionaries win!</B></FONT>"
	else if(finished == 2)
		feedback_set_details("round_end_result","loss - rev heads killed")
		text += "\red <br><FONT size = 3><B> The heads of staff managed to stop the revolution!</B></FONT>"
	..()
	outputstats += text
	return 1
...
/datum/game_mode/proc/auto_declare_completion_revolution()
	...
	if(head_revolutionaries.len || istype(ticker.mode,/datum/game_mode/revolution))
		var/text = "<br><br><FONT size = 2><B>The head revolutionaries were:</B></FONT>"
		...
		for(var/datum/mind/headrev in head_revolutionaries)
			...
			if(headrev.current)
				if(headrev.current.stat == DEAD)
					text += "died"
				else if(headrev.current.z != 1 || !headrev.current.client)
	...
	if(revolutionaries.len || istype(ticker.mode,/datum/game_mode/revolution))
		var/text = "<br><br><FONT size = 2><B>The revolutionaries were:</B></FONT>"
		...
		for(var/datum/mind/rev in revolutionaries)
			...
			if(rev.current)
				if(rev.current.stat == DEAD)
					...
					text += "fled the station"
				else if(!rev.current.client)//Logged out
					text += "logged out"
				else
					text += "survived the revolution"
	...
	if( head_revolutionaries.len || revolutionaries.len || istype(ticker.mode,/datum/game_mode/revolution) )
		var/text = "<br><br><FONT size = 2><B>The heads of staff were:</B></FONT>"
		...
		for(var/datum/mind/head in heads)
			...
			if(head.current)
				if(head.current.stat == DEAD)//Died
					text += "died"
				else if(head.current.z != 1)//Left the Z level
					text += "fled the station"
				else if(!head.current.client)//Logged out
					text += "logged out"
				else
					text += "survived the revolution"
