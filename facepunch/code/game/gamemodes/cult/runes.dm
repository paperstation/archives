...
/////////////////////////////////////////FIRST RUNE
	proc
		...
		//	var/tempnum = 0
			...
			if(index >= 5)
				user << "\red The rune disappears in a reality shift caused by too much wear of space-time fabric"
				del(src)
		...
		convert()
			for(var/mob/living/carbon/M in src.loc)
				...
				if(is_convertable_to_cult(M.mind))
					...
					M << "<font color=\"purple\"><b>You are unaffected by the rune!</b></font>"
					return 0
...
/////////////////////////////////////////FOURTH RUNE
		...
		tearreality()
			...
			if(cultist_count >= 9)
				for(var/mob/M in range(1,src))
					if(iscultist(M) && !M.stat)
						M.unlock_achievement("Birth of a God")
			...
			if(drain>=50)
				...
				spawn()
					for(,user.bhunger>0,user.bhunger--)
						sleep(50)
						user.deal_damage(3, BRUTE)
				return
			user.deal_damage(-drain%5, BRUTE)
			drain-=drain%5
			for (,drain>0,drain-=5)
				sleep(2)
				user.deal_damage(-5, BRUTE)
			return
...
/////////////////////////////////////////EIGHTH RUNE
		...
		raise()
			...
			corpse_to_raise.deal_overall_damage(-1000,-1000)
			corpse_to_raise.tox_damage = 0
			corpse_to_raise.oxy_damage = 0
			corpse_to_raise.deal_damage(-100, PARALYZE)
...
/////////////////////////////////////////THIRTEENTH RUNE
		...
		mend()
			...
			user.deal_overall_damage(200, 200)
			runedec+=10
...
/////////////////////////////////////////SEVENTEENTH RUNE
		...
		wall()//There should likely be a way to remove these walls as a noncult
		...
		bloodboil() //cultists need at least one DANGEROUS rune. Even if they're all stealthy.
			...
			if(culcount>=3)
				for(var/mob/living/carbon/M in viewers(usr))
					...
					if(N)
						continue
					M.deal_overall_damage(51,51)//this might be a bit high
...
// WIP rune, I'll wait for Rastaf0 to add limited blood.
		...
		burningblood()
			for(var/mob/living/M in orange(1,src))
				M.deal_damage(200, BURN)
			for(var/obj/effect/rune/R in world)
				for(var/mob/living/M in orange(2,R))
					M.deal_damage(20, BURN)
			...
			if(istype(src,/obj/effect/rune))   ///When invoked as rune, flash and stun everyone around.
				...
				for(var/mob/living/L in viewers(src))
					if(iscarbon(L))
						...
						C.deal_damage(1, WEAKEN)
						C.show_message("\red The rune explodes in a bright flash.", 3)

					else if(issilicon(L))
						var/mob/living/silicon/S = L
						S.show_message("\red The rune explodes in a bright flash.", 3)
				del(src)
				...
				if(issilicon(T))
					T.deal_damage(15, PARALYZE)

				else if(iscarbon(T))
					var/mob/living/carbon/C = T
					flick("e_flash", C.flash)
					C.silent += 15
					C.deal_damage(25, WEAKEN)
					C.deal_damage(25, STUTTER)
				return
