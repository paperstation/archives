...
var/global/cultteam1 = 0
var/global/cultteam2 = 0
...
/proc/is_convertable_to_cult(datum/mind/mind)
	...
	if(istype(mind.current, /mob/living/carbon/human) && (mind.assigned_role in list("Captain", "Head of Security", "Head of Personnel")))	return 0
...
/datum/game_mode/cult
	name = "cult"
	config_tag = "cult"
	restricted_jobs = list("AI", "Cyborg", "Security Officer", "Warden", "Detective", "Head of Security", "Head of Personnel", "Captain")
	protected_jobs = list()
	required_players = 15
	required_enemies = 3
...
/proc/gumbotesting()//TOGGLE
	if(ticker.mode:gumbotesting == 1)
		message_admins("Cult vs Cult testing has been enabled.")
		ticker.mode:gumbotesting = 0
	else
		message_admins("Cult vs Cult testing has been disabled.")
		ticker.mode:gumbotesting = 1
/datum/game_mode/cult/process()
	if(gumbotesting)
		if(team1state == 0 && cultteam1 >= 100)
			for(var/turf/simulated/wall/S in world)
				S.icon_state = "cult01"
			team1state = 1
		if(team2state == 0 && cultteam2 >= 100)
...
/datum/game_mode/cult/post_setup()
	...
	for(var/datum/mind/cult_mind in cult)
		...
		grant_startingwords(cult_mind.current)
...
/datum/game_mode/cult/proc/grant_startingwords(var/mob/living/carbon/human/cult_mob)
	for(var/startword in startwords)
		grant_runeword(cult_mob, startword)
	return


/datum/game_mode/cult/grant_runeword(var/mob/living/carbon/human/cult_mob, var/word)
	if(!word)
		for(var/startword in startwords)
			word = startword
...
/datum/game_mode/cult/declare_completion()
	...
	if(!check_cult_victory())
		...
		text += "\red <br><FONT size = 3><B> The cult wins! It has succeeded in serving its dark masters!</B></FONT>"
	else
		feedback_set_details("round_end_result","loss - staff stopped the cult")
		feedback_set("round_end_result",acolytes_survived)
		text += "\red <br><FONT size = 3><B> The staff managed to stop the cult!</B></FONT>"

		text += "<br><b>Cultists escaped:</b> [acolytes_survived]"

	if(objectives.len)
		text += "<br><br><b>The cultists' objectives were:</b>"
		for(var/obj_count=1, obj_count <= objectives.len, obj_count++)
			var/explanation
			switch(objectives[obj_count])
				if("survive")
