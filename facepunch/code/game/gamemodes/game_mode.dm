...
/datum/game_mode
	...
	var/votable = 1
	var/probability = 1
	var/station_was_nuked = 0 //The nuke went off, this var == the z level blown up
	...
	var/uplink_special_items = 1//toggles job specific traitor items
...
/obj/item/weapon/gun/energy/crossbow:6:Energy Crossbow;
...
/obj/item/weapon/gun/energy/disabler:3:Disabler;
Whitespace:Seperator;
Stealthy and Inconspicuous Weapons;
/obj/item/weapon/pen/paralysis:3:Paralysis Pen;
/obj/item/weapon/soap/syndie:1:Syndicate Soap;
/obj/item/weapon/cartridge/syndicate/tracking:2:Tracking PDA Cartridge;
Whitespace:Seperator;
...
/obj/item/weapon/storage/box/syndie_kit/imp_explode:2:Explosive Implant;
Whitespace:Seperator;
(Pointless) Badassery;
...
/datum/game_mode/proc/declare_completion()
	...
	if(surviving_humans > 0)
		...
		outputstats += "<br>[surviving_humans] human(s) survived."
	if(surviving_total > 0)
		feedback_set("survived_total",surviving_total)
	if(escaped_humans > 0)
		feedback_set("escaped_human",escaped_humans)
		outputstats += "<br>[escaped_humans] human(s) escaped."
	if(escaped_total > 0)
		feedback_set("escaped_total",escaped_total)
	if(escaped_on_shuttle > 0)
		feedback_set("escaped_on_shuttle",escaped_on_shuttle)
		outputstats += "<br>[escaped_on_shuttle] human(s) escaped on the shuttle."
	if(escaped_on_pod_1 > 0)
		feedback_set("escaped_on_pod_1",escaped_on_pod_1)
		outputstats += "<br>[escaped_on_pod_1] human(s) escaped on pod 1."
	if(escaped_on_pod_2 > 0)
		feedback_set("escaped_on_pod_2",escaped_on_pod_2)
		outputstats += "<br>[escaped_on_pod_2] human(s) escaped on pod 2."
	if(escaped_on_pod_3 > 0)
		feedback_set("escaped_on_pod_3",escaped_on_pod_3)
		outputstats += "<br>[escaped_on_pod_3] human(s) escaped on pod 3."
	if(escaped_on_pod_5 > 0)
		feedback_set("escaped_on_pod_5",escaped_on_pod_5)
		outputstats += "<br>[escaped_on_pod_5] human(s) escaped on pod 5."
	outputstats += "<br><br>Gamemode statistics:"
...
/datum/game_mode/proc/get_players_for_role(var/role, override_jobbans=1)
	...
	//	if(BE_SAMURAI)		roletext="samurai"
