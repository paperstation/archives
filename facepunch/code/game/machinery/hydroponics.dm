...
obj/machinery/hydroponics/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if(istype(O, /obj/item/weapon/reagent_containers/glass/bucket))
		...
		update_icon()
/*
	else if(istype(O, /obj/item/weapon/nutrientstor))
		var/obj/item/weapon/nutrientstor/myNut = O
		if(myNut.uses >= 1)
			if(myNut.mixed == 1)
				adjustNutri(10)
				yieldmod = myNut.yieldmod
				mutmod = myNut.mutmod
				user << "You spray some nutrient from the [myNut.name] in to the [src]."
				myNut.uses -= 1
*/
...
/obj/machinery/hydroponics/proc/adjustHealth(var/adjustamt)
	plant_health += adjustamt
	plant_health = max(plant_health, 0)
	plant_health = min(plant_health, myseed.endurance)
...
/obj/item/seeds/alienplantseed/harvest(mob/user = usr)
	var/obj/machinery/hydroponics/parent = loc //for ease of access
	parent.visible_message("\red The Maneater bursts apart the Hydroponics tray!")
	new /obj/effect/decal/cleanable/dirt(parent.loc)
	new /obj/effect/decal/cleanable/dirt(parent.loc)
	new /obj/effect/decal/cleanable/dirt(parent.loc)
	new /obj/effect/decal/cleanable/greenglow(parent.loc)
	new /obj/effect/decal/cleanable/generic(parent.loc)
	new /mob/living/simple_animal/alienplant/(usr.loc)
	sleep(1)
	del(parent)