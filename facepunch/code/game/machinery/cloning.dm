...
/obj/item/weapon/implant/health/proc/sensehealth()
	if (!src.implanted)
		...
		if(isliving(src.implanted))
			...
			src.healthstring = "[round(L.oxy_damage)] - [round(L.get_fire_loss())] - [round(L.tox_damage)] - [round(L.get_brute_loss())]"
		if (!src.healthstring)
...
/obj/machinery/clonepod/proc/growclone(var/ckey, var/clonename, var/ui, var/se, var/mindref, var/mrace)
	...
	H.deal_damage(src.heal_level + 100, CLONE) //new damage var so you can't eject a clone early then stab them to abuse the current damage system --NeoFite
	//Here let's calculate their health so the pod doesn't immediately eject them!!!
	H.deal_damage(10, PARALYZE)//Stun them right away
...
/obj/machinery/clonepod/process()
	...
	if((src.occupant) && (src.occupant.loc == src))
		if((src.occupant.stat == DEAD) || (src.occupant.suiciding) || !occupant.key)  //Autoeject corpses and suiciding dudes.
			...
			src.occupant.deal_damage(4, PARALYZE)

			 //Slowly get that clone healed and finished.
			occupant.deal_damage(-2, CLONE)

			//Premature clones may have brain damage.
			occupant.brain_damage = max(occupant.brain_damage-2,0)
			...
			src.occupant.deal_damage(-4, OXY)
