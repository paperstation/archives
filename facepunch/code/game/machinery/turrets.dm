...
/obj/machinery/turretid
	...
	#ifdef NEWMAP
	req_access = list(access_ai_area)
	#else
...
/obj/structure/turret/gun_turret
	...
	var/on = 0
	var/projectile_type = "/obj/item/projectile/hivebotbullet"
	icon = 'icons/obj/turrets.dmi'
...
/obj/structure/turret/autogun/cult
	name = "Autogun"
	density = 1
	anchored = 1
	var/cooldown = 20
	var/projectiles = 10000000000000000
	var/projectiles_per_shot = 2
	var/deviation = 0.3
	var/list/exclude = list()
	var/atom/cur_target
	...
	health = 40
	max_health = 40
	var/list/scan_for = list("human"=1,"cyborg"=1,"mecha"=1,"alien"=1)
	var/on = 1
	var/projectile_type = "/obj/item/projectile/hivebotbullet"
	icon = 'icons/obj/turrets.dmi'
	icon_state = "gun_turret"
	...
	proc/get_target()
		...
		if(scan_for["human"])
			for(var/mob/living/carbon/human/M in oview(scan_range,src))
				if(M.stat || M.lying || M in exclude)
					continue
				if(locate(/obj/item/clothing/suit/cultrobes) in M || !locate(/obj/item/clothing/head/culthood) in M)
					continue
				pos_targets += M
		if(scan_for["cyborg"])
			for(var/mob/living/silicon/M in oview(scan_range,src))
