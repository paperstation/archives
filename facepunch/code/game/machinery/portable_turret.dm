...
/obj/machinery/porta_turret
	...
	density = 1
	use_power = USE_POWER_IDLE			// this turret uses and requires power
...
"<A href='?src=\ref[src];operation=checkxenos'>[check_anomalies ? "Yes" : "No"]</A>" )
		...
		if(istype(user,/mob/living/carbon/human))
			...
			if(((src.lasercolor) == "b") && (istype(H.wear_suit, /obj/item/clothing/suit/lasertag/red)))
				return
			if(((src.lasercolor) == "r") && (istype(H.wear_suit, /obj/item/clothing/suit/lasertag/blue)))
				return
		dat += text({"
<TT><B>Automatic Portable Turret Installation</B></TT><BR><BR>
Status: []<BR>"},
	...
	if(on)
		if(!attacked && !emagged)
			...
			spawn()
				sleep(60)
				attacked = 0
	if(prob(45) && Proj.force > 0) src.spark_system.start()
	..()
	return
...
/obj/machinery/porta_turret/destroy() // called when the turret dies, ie, health <= 0
	src.health = 0
	src.density = 0
...
/obj/machinery/porta_turret/proc/assess_perp(mob/living/carbon/human/perp as mob)
	...
	if((src.lasercolor) == "b")//Lasertag turrets target the opposing team, how great is that? -Sieve
		...
		if(istype(perp.wear_suit, /obj/item/clothing/suit/lasertag/red))
			threatcount += 4


	if((src.lasercolor) == "r")
		threatcount = 0
		if(istype(perp.wear_suit, /obj/item/clothing/suit/lasertag/blue))
			threatcount += 4
...
"<A href='?src=\ref[src];operation=checkxenos'>[Parent_Turret.check_anomalies ? "Yes" : "No"]</A>" )
		...
		if(istype(user,/mob/living/carbon/human))
			var/mob/living/carbon/human/H = user
			if(((Parent_Turret.lasercolor) == "b") && (istype(H.wear_suit, /obj/item/clothing/suit/lasertag/red)))
				return
			if(((Parent_Turret.lasercolor) == "r") && (istype(H.wear_suit, /obj/item/clothing/suit/lasertag/blue)))
				return
		dat += text({"
<TT><B>Automatic Portable Turret Installation</B></TT><BR><BR>
Status: []<BR>"},
