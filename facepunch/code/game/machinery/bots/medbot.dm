...
/obj/machinery/bot/medbot/assx
	name = "Inspection Medibot"
	desc = "He sells Ass-x and Ass-x accessories."
	skin = "bezerk"
	treatment_oxy = "dexalinp"
	treatment_brute = "bicaridine"
	treatment_fire = "kelotane"
...
/obj/machinery/bot/medbot/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if (istype(W, /obj/item/weapon/card/id)||istype(W, /obj/item/device/pda))
		...
		..()
		if (health < max_health && !istype(W, /obj/item/weapon/screwdriver) && W.force)
...
/obj/machinery/bot/medbot/proc/assess_patient(mob/living/carbon/C as mob)
	...
	if((src.reagent_glass) && (src.use_beaker) && ((C.get_brute_loss() >= heal_threshold) || (C.get_fire_loss() >= heal_threshold) || (C.tox_damage >= heal_threshold) || (C.oxy_damage >= (heal_threshold + 15))))
	...
	if((C.get_brute_loss() >= heal_threshold) && (!C.reagents.has_reagent(src.treatment_brute)))		return 1 //If they're already medicated don't bother!

	if((C.oxy_damage >= (15 + heal_threshold)) && (!C.reagents.has_reagent(src.treatment_oxy)))
		return 1

	if((C.get_fire_loss() >= heal_threshold) && (!C.reagents.has_reagent(src.treatment_fire)))
		return 1

	if((C.tox_damage >= heal_threshold) && (!C.reagents.has_reagent(src.treatment_tox)))
		return 1
...
/obj/machinery/bot/medbot/destroy()
	src.on = 0
