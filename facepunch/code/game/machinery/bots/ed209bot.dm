...
/obj/machinery/bot/ed209/New(loc, created_name)
	..()
		...
		if(SECBOT_HUNT)		// hunting for perp
			...
			if (target)		// make sure target exists
				if (get_dist(src, src.target) <= 1)		// if right next to perp
					...
					M.deal_damage(10, STUTTER, IMPACT, "chest")
					maxstuns--
					if(maxstuns <= 0)
						target = null
...
/obj/machinery/bot/ed209/destroy()
	walk_to(src,0)
	...
	switch(build_step)
		...
		if(9)
			if( istype(W, /obj/item/weapon/cell) )
				...
				new /obj/machinery/bot/ed209(T,created_name)
				user.drop_item()
				del(W)
