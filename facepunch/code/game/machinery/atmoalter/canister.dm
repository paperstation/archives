...
/obj/machinery/portable_atmospherics/canister/update_icon()
	...
	if (src.destroyed)
		src.icon_state = text("[]-1", src.variant)
...
/obj/machinery/portable_atmospherics/canister/update_health()
	if(!destroyed && (health <= max_health * 0.2))
		destroyed = 1
	...
	if (((get_dist(src, usr) <= 1) && istype(src.loc, /turf)))
		...
		if (href_list["relabel"])
			if (can_label)
				...
				var/label = input("Choose canister label", "Gas canister") as null|anything in variants
				if (label)
					src.variant = variants[label]
					src.icon_state = variants[label]
...
/obj/machinery/portable_atmospherics/canister/oxy_b
