...
/obj/machinery/sleep_console/attack_hand(mob/user as mob)
	...
	if (src.connected)
		...
		if (occupant)
			...
			dat += text("[]\t-Brute Damage %: []</FONT><BR>", (occupant.get_brute_loss() < 60 ? "<font color='blue'>" : "<font color='red'>"), occupant.get_brute_loss())
			dat += text("[]\t-Respiratory Damage %: []</FONT><BR>", (occupant.oxy_damage < 60 ? "<font color='blue'>" : "<font color='red'>"), occupant.oxy_damage)
			dat += text("[]\t-Toxin Content %: []</FONT><BR>", (occupant.tox_damage < 60 ? "<font color='blue'>" : "<font color='red'>"), occupant.tox_damage)
			dat += text("[]\t-Burn Severity %: []</FONT><BR>", (occupant.get_fire_loss() < 60 ? "<font color='blue'>" : "<font color='red'>"), occupant.get_fire_loss())
			...
			if(emagged)
				dat += text("<HR><BR><A href='?src=\ref[];tox=1'>Inject Toxin</A>", src)
		else
...
/obj/machinery/sleep_console/attackby(obj/item/weapon/W, mob/user)
	if (istype(W, /obj/item/weapon/card/emag) && !emagged)
		user << "\red You short out the extra chemical storage lock."
		emagged = 1
		...
		if (src.connected)
			if (src.connected.occupant)
				if(src.connected.occupant.health > 0)
					...
					if(emagged == 1)
						if (href_list["tox"])
							src.connected.inject_toxin(usr)
				else
...
/obj/machinery/sleeper
	...
	proc/inject_toxin(mob/living/user as mob)
		if(src.occupant && src.occupant.reagents)
			if(src.occupant.reagents.get_reagent_amount("toxin") + 30 <= 60)
				src.occupant.reagents.add_reagent("toxin", 30)
			user << text("Occupant now has [] units of Toxin in his/her bloodstream.", src.occupant.reagents.get_reagent_amount("toxin"))
		else
			user << "There's no occupant in the sleeper or the subject rejects the chemicals!"
		return
	...
	proc/check(mob/living/user as mob)
		if(src.occupant)
			...
			user << text("[]\t -Brute Damage %: []", (src.occupant.get_brute_loss() < 60 ? "\blue " : "\red "), src.occupant.get_brute_loss())
			user << text("[]\t -Respiratory Damage %: []", (src.occupant.oxy_damage < 60 ? "\blue " : "\red "), src.occupant.oxy_damage)
			user << text("[]\t -Toxin Content %: []", (src.occupant.tox_damage < 60 ? "\blue " : "\red "), src.occupant.tox_damage)
			user << text("[]\t -Burn Severity %: []", (src.occupant.get_fire_loss() < 60 ? "\blue " : "\red "), src.occupant.get_fire_loss())
