...
/obj/machinery/atmospherics/unary/cryo_cell/attack_hand(mob/user as mob)
	...
	var/dat = {"<B>Cryo cell control system</B><BR>
		...
		[beaker_text]<BR><BR>
		<B>Current occupant:</B> [occupant ? "<BR>Name: [occupant]<BR>Health: [health_text]<BR>Oxygen deprivation: [round(occupant.oxy_damage,0.1)]<BR>Brute damage: [round(occupant.get_brute_loss(),0.1)]<BR>Fire damage: [round(occupant.get_fire_loss(),0.1)]<BR>Toxin damage: [round(occupant.tox_damage,0.1)]<BR>Body temperature: [occupant.bodytemperature]" : "<FONT color=red>None</FONT>"]<BR>
		"}
	user.set_machine(src)
...
/obj/machinery/atmospherics/unary/cryo_cell/proc/process_occupant()
	...
	if(occupant)
		...
		if(occupant.bodytemperature < T0C)
			occupant.sleeping = 5
			occupant.deal_damage(5, PARALYZE)
			if(air_contents.oxygen > 1)
				occupant.deal_damage(-5, OXY)
			//severe damage should heal waaay slower without proper chemicals
			if(occupant.bodytemperature < 225)
				occupant.deal_overall_damage(-2, -2)
...
/obj/machinery/atmospherics/unary/cryo_cell/verb/move_eject()
	...
	if(usr == occupant)//If the user is inside the tube...
		if (usr.stat == 2)//and he's not dead....
			return
		usr << "\blue Release sequence activated. This will take 30 seconds."
		sleep(300)
