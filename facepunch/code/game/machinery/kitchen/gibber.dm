...
/obj/machinery/gibber/attackby(obj/item/weapon/grab/G as obj, mob/user as mob)
	if(src.occupant)
		...
		return
	if(istype(G, /obj/item/weapon/disk/chefupgrade))
		if(!chems)
			chems = 1
			user << "Initiating dangerious chemical storages. Meat will now be contaminated with a variety dangerious chemicals!"
			user.drop_item(G)
			del(G)
	...
	user.visible_message("\red [user] starts to put [G.affecting] into the [src]!")
	src.add_fingerprint(user)
	if(do_after(user, 30) && G && G.affecting && !occupant)
		user.visible_message("\red [user] stuffs [G.affecting] into the [src]!")
...
/obj/machinery/gibber/proc/startgibbing(mob/user as mob)
	...
	for (var/i=1 to totalslabs)
		...
		if(chems)//Sneaky sneaky
			if(prob(15))
				newmeat.reagents.add_reagent ("chloralhydrate", sourcenutriment / totalslabs)
			else if(prob(25))
				newmeat.reagents.add_reagent ("mutagen", sourcenutriment / totalslabs)
			else if(prob(35))
				newmeat.reagents.add_reagent ("stoxin", sourcenutriment / totalslabs)
			else if(prob(45))
				newmeat.reagents.add_reagent ("space_drugs", sourcenutriment / totalslabs)
			else
				newmeat.reagents.add_reagent ("toxin", sourcenutriment / totalslabs)
	...
	user.unlock_achievement("Slab of Heaven")
