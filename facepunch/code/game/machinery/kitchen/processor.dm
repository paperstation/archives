...
/obj/machinery/monkeygrinder
	name = "Bone Grinder"
	desc = "Its a good thing this isn't an organ grinder."
	icon = 'icons/obj/kitchen.dmi'
	icon_state = "grinder"
	layer = 2.9
	density = 1
	...
	attack_hand()
		if(!on)
			on = 1
			usr.unlock_achievement("The Cycle of Life")
		else
			on = 0

/obj/machinery/monkeygrinder/process()
	if(on)
		for(var/obj/effect/decal/remains/human/T in range(src, 1))
