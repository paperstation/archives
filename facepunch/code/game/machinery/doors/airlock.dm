...
/proc/RandomAirlockWires()
	...
	var/flagIndex = 1
	for (var/flag=1, flag<2047, flag+=flag)
		var/valid = 0
		...
		AIRLOCK_WIRE_SAFETY, AIRLOCK_WIRE_SPEED)
...
/obj/machinery/door/airlock
	...
	New()
		...
		if(!A)
			return
		//If we have the default name then take the area name and use that
		if(src.name == initial(name))
			name = "[A.name]"
		//Next up if we have no access then we should set the area default if it has one
		if(!req_access.len && A.default_access_req)
			var/list/req_access_str = text2list(A.default_access_req,";")//Split the
			if(!req_access)
				req_access = list()
...
/obj/machinery/door/airlock/maintenance
	...
	New()
		..()
		//If our name used the autoname code then we should generally append Maintenance onto it
		var/area/A = get_area(src.loc)
		if(!A)
			return
		if(src.name == A.name)
			//If this area is not a maintenance area then we should add Maintenance to the doorname ie: Detective + Maintenance rather than just detective.
...
/obj/machinery/door/airlock/animate_icon(animation)
	switch(animation)
		if("opening")
			if(overlays) overlays.Cut()
			if(p_open)
...
/obj/machinery/door/airlock/attack_hand(mob/user as mob)
	...
	if(ishuman(user) && prob(40) && src.density)
		...
		if(H.brain_damage >= 60)
			...
			if(!istype(H.head, /obj/item/clothing/head/helmet))
				...
				H.deal_damage(5, WEAKEN)
				H.deal_damage(10, BRUTE, IMPACT, "head")
			else
	...
	if(src.p_open)
		...
		t1 += text("<br>\n[]<br>\n[]<br>\n[]<br>\n[]<br>\n[]", (src.locked ? "The door bolts have fallen!" : "The door bolts look up."), ((src.arePowerSystemsOn() && !(stat & NOPOWER)) ? "The test light is on." : "The test light is off!"), (src.aiControlDisabled==0 ? "The 'AI control allowed' light is on." : "The 'AI control allowed' light is off."),  (src.safe==0 ? "The 'Check Wiring' light is on." : "The 'Check Wiring' light is off."), (src.normalspeed==0 ? "The 'Check Timing Mechanism' light is on." : "The 'Check Timing Mechanism' light is off."))
...
/obj/machinery/door/airlock/attackby(C as obj, mob/user as mob)
	...
	if((istype(C, /obj/item/weapon/weldingtool) && !( src.operating ) && src.density))
		...
		if(density && arePowerSystemsOn() && !(stat & NOPOWER))//Prevent crowbaring if the power is on, doorwires are fine, and its closed.
			user << "\blue The airlock's motors resist your efforts to force it."
			return

		//At this point the door is not locked, has power, and its wires are fine.  So we make sure its not already doing something and that, if closed, is not welded.  Open doors can be crowbar'd closed just fine.
		if((!density || !welded) && !operating)//if open or not welded and not already doing things then function.
			if(density)//This fireaxe thing is quite silly but I will leave it here for now
		...
		if(istype(src, /obj/machinery/door/airlock/glass))
			...
			for(var/mob/M in hearers(15, source))
				...
				if(M_turf && M_turf.z == source.z)
					...
					M.playsound_local(source, 'sound/machines/windowdoor.ogg', far_volume, 1, 1, falloff = 5)
		else
			var/turf/source = get_turf(src)
			var/far_dist = 0
			for(var/mob/M in hearers(15, source))
				...
				if(M_turf && M_turf.z == source.z)
					...
					var/far_volume = Clamp(far_dist, 30, 50)
					far_volume += (dist <= far_dist * 0.5 ? 50 : 0)
					M.playsound_local(source, 'sound/machines/airlock.ogg', far_volume, 1, 1, falloff = 3)
	else
		var/turf/source = get_turf(src)
		var/far_dist = 0
		for(var/mob/M in hearers(15, source))
			...
			if(M_turf && M_turf.z == source.z)
				...
				var/far_volume = Clamp(far_dist, 30, 50)
				far_volume += (dist <= far_dist * 0.5 ? 50 : 0)
				M.playsound_local(source, 'sound/machines/door_forced.ogg', far_volume, 1, 1, falloff = 5)
...
/obj/machinery/door/airlock/close(var/forced=0)
	...
	for(var/mob/living/M in get_turf(src))
		if(isrobot(M))
			M.deal_damage(DOOR_CRUSH_DAMAGE, BRUTE, IMPACT)
		else
			M.deal_damage(DOOR_CRUSH_DAMAGE, BRUTE, IMPACT)
			M.emote("scream")
