		...
		switch(menustat)
			if("menu")
				if (beaker)
					...
					dat += "Material<BR>"
					dat += "<A href='?src=\ref[src];action=create;item=cloth;cost=250'>Cloth</A> <FONT COLOR=blue>(250)</FONT> | <A href='?src=\ref[src];action=create;item=cloth5;cost=1250'>x5</A><BR>"
...
/obj/machinery/biogenerator/proc/create_product(var/item,var/cost)
	...
	switch(item)
		...
		if("cloth5")
			var/obj/item/stack/sheet/cloth/B = new/obj/item/stack/sheet/cloth
			B.loc = src.loc
			B.amount = 5
		if("gloves")
