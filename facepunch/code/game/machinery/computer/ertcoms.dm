...
/obj/machinery/computer/communications/ert
	name = "ERT Communications Console"
	desc = "The ERT version of the communications console. Has a few more features.."
	circuit = "/obj/item/weapon/circuitboard/communications/ert"
	req_access = list(access_cent_specops)


/obj/machinery/computer/communications/ert/Topic(href, href_list)
	if(..())
		return
	if (src.z > 1)
		...
		if("announce")
			if(src.authenticated==2)
				...
				if(!input || !(usr in view(1,src)))
					return
				ert_announce(input)//This should really tell who is, IE HoP, CE, HoS, RD, Captain
				log_say("[key_name(usr)] has made a captain announcement: [input]")
				message_admins("[key_name_admin(usr)] has made an ERT announcement.", 1)
				message_cooldown = 1
...
/obj/machinery/computer/communications/ert/attack_hand(var/mob/user as mob)
	if(..())
		return
	if (src.z > 6)
	...
	var/dat = "<head><title>ERT Communications Console</title></head><body>"
