/obj/machinery/computer/robotics
	...
	icon_state = "robot"
	#ifdef NEWMAP
	req_access = list(access_cyborg_control)
	#else
		...
		if (src.temp)
			...
			if(screen == 1)
				for(var/mob/living/silicon/robot/R in mob_list)
					if(istype(user, /mob/living/silicon/ai))//AIs can only see the borgs they control
						if(R.connected_ai != user)
							continue
					if(istype(user, /mob/living/silicon/robot))//Cyborgs can only see its own control
						if(R != user)
							continue
					if(R.scrambledcodes)
						continue
					...
					/*if (istype(user, /mob/living/silicon)) Removed due to no longer doing anything
