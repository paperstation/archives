/obj/machinery/camera
	...
	proc/setViewRange(var/num = 7)
		src.view_range = num
		cameranet.updateVisibility(src, 0)
		return


	attack_paw(mob/living/carbon/alien/humanoid/user as mob)
		if(!istype(user))
			return
	...
	attackby(W as obj, mob/living/user as mob)
		if(iswirecutter(W))
			deactivate(user)
			return
		...
		if(iswelder(W))
			if(status)
				user << "<span class='notice'>You need to disable the camera first.</span>"
				return
			if(weld(W, user))
				visible_message("<span class='warning'>[user] cuts apart the [src]!</span>")
				if(assembly)
					assembly.loc = src.loc
					assembly.state = 1
				del(src)
	...
	proc/triggerCameraAlarm()
		alarm_on = 1
		for(var/mob/living/silicon/S in mob_list)
			S.triggerAlarm("Camera", get_area(src), list(src), src)


	proc/cancelCameraAlarm()
		alarm_on = 0
		for(var/mob/living/silicon/S in mob_list)
	...
	proc/weld(var/obj/item/weapon/weldingtool/WT, var/mob/user)
		if(busy)
			return 0
		if(!WT.isOn())
			return 0
