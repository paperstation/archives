...
/proc/domutcheck(mob/living/M as mob, connected, inj)
	...
	if (isblockon(getblock(M.dna.struc_enzymes, FIREBLOCK,3),10))
		if(inj || prob(30))
			...
			M.mutations.Add(TEMPATURE_RESIST)
...
//////////////////////////////////////////////////////////// Monkey Block
	...
	// human > monkey
		...
		O.deal_overall_damage(M.get_brute_loss() + 40, M.get_fire_loss())
		O.tox_damage = M.tox_damage + 20
		O.oxy_damage = M.oxy_damage
		O.stat = M.stat
		O.a_intent = "hurt"
...
//		O.update_icon = 1	//queue a full icon update at next life() call
	...
	// monkey > human,
		...
		O.deal_overall_damage(M.get_brute_loss(), M.get_fire_loss())
		O.tox_damage = M.tox_damage
		O.oxy_damage = M.oxy_damage
		O.stat = M.stat
		for (var/obj/item/weapon/implant/I in implants)
			I.loc = O
