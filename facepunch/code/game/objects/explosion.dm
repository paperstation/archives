...
//newexplosion
proc/newexplosion(turf/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, adminlog = 1, ignorecap = 0, fueltank = 0)
	src = null	//so we don't abort once src is deleted
	...
	spawn(0)
		...
		if(heavy_impact_range)
			for(var/mob/M in player_list)
				if(M.z != epicenter.z)
					continue
				M << sound('sound/effects/explosionfar.ogg')//Will play for everyone on the same Z level, might have issues on the away level
		playsound(epicenter, "sound/effects/fueltank.ogg", 100, 1, round(devastation_range,1) )


		--lighting_controller.can_fire	//Pause the lighting updates for a bit
		++SSpower.defer_powernet_rebuild
