...
/obj/structure/toilet/attack_hand(mob/living/user as mob)
	if(swirlie)
		...
		swirlie.deal_damage(8, BRUTE, IMPACT, "head")
		return
			...
			if(G.state>1)
				...
				if(open && !swirlie)
					...
					if(do_after(user, 30, 5, 0))
						...
						if(!GM.internal)
							GM.deal_damage(5, OXY)
					swirlie = null
				else
					user.visible_message("<span class='danger'>[user] slams [GM.name] into the [src]!</span>", "<span class='notice'>You slam [GM.name] into the [src]!</span>")
					GM.deal_damage(10, BRUTE, IMPACT, "head")
			else
...
/obj/item/weapon/bikehorn/rubberducky/attack_self(mob/user as mob)
	if (spam_flag == 0)
		spam_flag = 1
		playsound(src.loc, 'sound/items/ducky.ogg', 50, 1)
...
/*
/obj/structure/sink/darkroom
	name = "kitchen sink"
	icon_state = "sink_alt"

/obj/structure/sink/darkroom/attackby(obj/item/O as obj, mob/user as mob)

	..()
	if(istype(O, /obj/item/weapon/photo/))
		var/obj/item/weapon/photo/D = O
		if(D.progtype == 3)
			D.progtype = 4
