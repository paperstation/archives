...
//HUD things
	...
	var/can_remove = 1 //Mostly for Ninja code at this point but basically will not allow the item to be removed if set to 0. /N
		//This can likely be removed, only a few nonclothes use it
...
/obj/item/examine()
	...
	usr << "It appears to do [forcetype] damage and has \a [damagedesc] force."
...
/obj/item/attack_hand(mob/user as mob)
	...
	if(src.loc == user)
		...
		if(!src.can_remove)
			user << "\red You are unable to remove the [name]!"
			return
		else
			user.u_equip(src)
	else
...
/obj/item/attack_paw(mob/user as mob)
	...
	if (src.loc == user)
		//canremove==0 means that object may not be removed. You can still wear it. This only applies to clothing. /N
		if(istype(src, /obj/item/clothing) && !src:can_remove)
			return
		else
			user.u_equip(src)
	else
...
/obj/item/proc/attack(mob/living/M as mob, mob/living/user as mob, def_zone)
	...
	if(src.hit_sound)
		playsound(src.loc, pick(hit_sound), 50, 1, -1)
	add_fingerprint(user)
	...
	if(!istype(M, /mob/living/carbon/human))
		if(istype(M, /mob/living/carbon/slime))
			...
			S.was_attacked(user, force, def_zone)
	...
	if(istype(M, /mob/living/carbon/human))
		...
		return 1
	if(src.damtype == BRUTE)
		if(prob(force)) // Added blood for whacking non-humans too
			var/turf/location = M.loc
			if(istype(location, /turf/simulated))
				location.add_blood_floor(M)

	M.deal_damage(damage, damtype, forcetype, def_zone)
	return 1
...
/obj/item/proc/mob_can_equip(M as mob, slot, disable_warning = 0)
	...
	if(ishuman(M))
		...
		if(istype(src, /obj/item/clothing/suit))
			if(HULK in H.mutations)
				H << "\red You're too bulky to wear the [name]."
				return 0
...
/obj/item/proc/eyestab(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
	...
	M.deal_damage(5, BRUTE, PIERCE, "head")
	...
	if (M.eye_stat >= 10)
		...
		if(prob(50))
			if(M.stat != 2)
				...
				M.drop_item()
			M.eye_blurry += 10
			M.deal_damage(4, WEAKEN, PIERCE, "head")
