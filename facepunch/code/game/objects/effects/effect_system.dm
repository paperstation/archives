...
/obj/effect/effect/mustard_gas/Move()
	...
	for(var/mob/living/carbon/human/R in get_turf(src))
		if(R.internal != null && usr.wear_mask && (R.wear_mask.flags & MASKINTERNALS) && R.wear_suit != null)
			return
		R.burn_skin(0.75)
		if(R.coughedtime != 1)
			R.coughedtime = 1
...
/obj/structure/foamedmetal
	...
	attack_hand(var/mob/user)
		if((HULK in user.mutations) || (prob(75 - metal*25)))
			...
			user.visible_message("\red [user] smashes through the foamed metal.")
			del(src)
		else
	...
	attackby(var/obj/item/I, var/mob/user)
		if (istype(I, /obj/item/weapon/grab))
			...
			user.visible_message("\red [G.assailant] smashes [G.affecting] through the foamed metal wall.")
			del(I)
			del(src)
			return
