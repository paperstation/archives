...
/obj/item/device/paicard/interact(mob/user)
	...
	if(pai)
		...
		if(looking_for_personality)
			...
			dat += {"
<style type="text/css">p.top {background-color: #AAAAAA; color: black;}
tr.d0 td {background-color: #CC9999; color: black;}
tr.d1 td {background-color: #9999CC; color: black;}
tr.d2 td {background-color: #99CC99; color: black;}
</style>
"}
			...
			dat += "<table>"
			var/list/available = get_candidates(BE_PAI)
			for(var/client/C in available)
				var/datum/preferences/P = C.prefs
				dat += "<tr class='d0'><td>Name:</td><td>[P.pai_name]</td></tr>"
				dat += "<tr class='d1'><td>Description:</td><td>[P.pai_description]</td></tr>"
				//dat += "<tr class='d0'><td>Preferred Role:</td><td>[P.pai_role]</td></tr>"
				//dat += "<tr class='d1'><td>OOC Comments:</td><td>[P.pai_comments]</td></tr>"
				dat += "<tr class='d2'><td><a href='byond://?src=\ref[src];download=1;candidate=\ref[P];'>\[Download [P.pai_name]\]</a></td><td></td></tr>"
			dat += "</table>"
...
/obj/item/device/paicard/Topic(href, href_list)
	...
	if(href_list["wires"])
		...
		if(href_list["setdna"] && !pai.master_dna)
			var/mob/M = usr
		...
		if(href_list["download"])
			var/datum/preferences/P = locate(href_list["candidate"])
			var/ckey
			for(var/t in preferences_datums)
				if(preferences_datums[t] == P)
					ckey = t
					break
			if(ckey && P.be_special & BE_PAI)
				var/mob/living/silicon/pai/M = new(src)
				M.name = P.pai_name ? P.pai_name : pick(ninja_names)
				M.real_name = "pAI [M.name]"
				M.ckey = ckey

				setPersonality(M)
				looking_for_personality = 0

				ticker.mode.update_cult_icons_removed(M.mind)
				ticker.mode.update_rev_icons_removed(M.mind)
