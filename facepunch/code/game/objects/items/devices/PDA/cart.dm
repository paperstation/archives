...
<a href='byond://?src=\ref[src];choice=Signal Code;scode=5'>+</a><br>"}
			...
			if (433) //Muskets' and Rockdtben's power monitor :D
				...
				if(!powmonitor)
					...
					menu += "<PRE>Total power: [num2text(round(powmonitor.powernet.avail,1),10)] W<BR>Total load:  [num2text(round(powmonitor.powernet.load,1),10)] W<BR>"
			...
			if (47) //quartermaster order records
				...
				menu += "Location: [supply_shuttle.moving ? "Moving to station ([round((supply_shuttle.eta_time - world.time)/600,1)] Mins.)":supply_shuttle.at_station ? "Station":"Dock"]<BR>"
