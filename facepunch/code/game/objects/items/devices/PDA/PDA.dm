...
//Holy fuck this setup is bad, the cart code should be in the actual cart and not all in these fucking massive switch statements.
			...
			if (2)
				...
				if(!toff)
					for(var/obj/item/device/pda/P in sortAtom(PDAs))
						...
						if(istype(cartridge, /obj/item/weapon/cartridge/syndicate/tracking))
							var/turf/T = get_turf(P)
							if(!T)	continue
							if(!T.loc || T.z > 5)	continue
							dat += "<br>Location: [T.loc.name] "
	...
	if (istype(C, /mob/living/carbon))
		switch(scanmode)
			if(1)
				...
				user.show_message("\blue \t Overall Status: [C.stat > 1 ? "dead" : "[C.health - C.fatigue]% healthy"]", 1)
				user.show_message("\blue \t Damage Specifics: [C.oxy_damage > 50 ? "\red" : "\blue"][C.oxy_damage]-[C.tox_damage > 50 ? "\red" : "\blue"][C.tox_damage]-[C.get_fire_loss() > 50 ? "\red" : "\blue"][C.get_fire_loss()]-[C.get_brute_loss() > 50 ? "\red" : "\blue"][C.get_brute_loss()]", 1)
				user.show_message("\blue \t Key: Suffocation/Toxin/Burns/Brute", 1)
				user.show_message("\blue \t Body Temperature: [C.bodytemperature-T0C]&deg;C ([C.bodytemperature*1.8-459.67]&deg;F)", 1)
				if(C.tod && C.stat == DEAD)
...
/mob/living/silicon/ai/proc/init_ai_pda()
	if(!aiPDA)
