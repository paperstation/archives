...
/obj/item/clothing/mask/cigarette/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/weapon/weldingtool))
		...
		if(C.lit)
			light("<span class='rose'>After some fiddling, [user] manages to light their [name] with [W].</span>")
	...
	attack_self(mob/living/user)
		...
		if(user.r_hand == src || user.l_hand == src)
			if(!lit)
				...
				icon_state = icon_on
				item_state = icon_on
				if(istype(src, /obj/item/weapon/lighter/zippo) )//Bleh more child object checking in a parent
					...
					if(prob(75))
						...
						user.deal_damage(5, BURN, PIERCE, "arms")
