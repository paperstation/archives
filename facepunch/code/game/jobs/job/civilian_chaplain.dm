...
/datum/job/chaplain
	...
	#ifdef NEWMAP
	access = list(access_morgue_area, access_chapel_area)
	#else
	...
	equip(var/mob/living/carbon/human/H)
		...
		var/obj/item/weapon/storage/bible/B = new /obj/item/weapon/storage/bible/special(H)
		...
		spawn(0)
			...
			switch(lowertext(new_religion))
				...
				if("lol", "wtf", "gay", "penis", "ass", "poo", "badmin", "shitmin", "deadmin", "cock", "cocks")
					...
					H.brain_damage += 100 // starts off retarded as fuck
				if("science")
					B.name = pick("Principle of Relativity", "Quantum Enigma: Physics Encounters Consciousness", "Programming the Universe", "Quantum Physics and Theology", "String Theory for Dummies", "How To: Build Your Own Warp Drive", "The Mysteries of Bluespace", "Playing God: Collector's Edition")
				else
					B.name = "The Holy Book of [new_religion]"
			religionname = new_religion
			...
			while(!accepted)
				...
				switch(new_book_style)
					if("Koran")
						B.icon_state = "koran"
						B.item_state = "koran"
						for(var/area/station/general/chapel/main/A in world)
