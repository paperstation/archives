...
/obj/machinery/biogenerator/proc/create_coin(var/item,var/cost)
	...
	switch(item)
		...
		if("steak")
			...
			B.loc = src.loc
		if("armband")
			var/obj/item/clothing/tie/armband/hydro/B = new/obj/item/clothing/tie/armband/hydro
			B.name = "Mr. Sprinkles Hippy Armband"
			B.desc = "THE COLORS!"
			B.loc = src.loc
		if("lipstick")
		...
		if("vip")
			...
			B.desc = "Walk up to a rope barrier that doesn't have any walls and use it on it to go through! No unwanted guests though"
			B.loc = src.loc
		if("bucket")
