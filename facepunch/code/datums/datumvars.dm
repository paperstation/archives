	...
	proc/debug_variables(datum/D in world)
		...
		if(istype(D,/atom))
			...
			if(isliving(A))
				...
				body += {"
				<br><font size='1'>
				BRUTE:<font size='1'><a href='?_src_=vars;mobToDamage=\ref[D];adjustDamage=brute'>[M.get_brute_loss()]</a>
				FIRE:<font size='1'><a href='?_src_=vars;mobToDamage=\ref[D];adjustDamage=fire'>[M.get_fire_loss()]</a>
				TOXIN:<font size='1'><a href='?_src_=vars;mobToDamage=\ref[D];adjustDamage=toxin'>[M.tox_damage]</a>
				OXY:<font size='1'><a href='?_src_=vars;mobToDamage=\ref[D];adjustDamage=oxygen'>[M.oxy_damage]</a>
				CLONE:<font size='1'><a href='?_src_=vars;mobToDamage=\ref[D];adjustDamage=clone'>[M.clone_damage]</a>
				BRAIN:<font size='1'><a href='?_src_=vars;mobToDamage=\ref[D];adjustDamage=brain'>[M.brain_damage]</a>
				</font>
...
/client/proc/view_var_Topic(href, href_list, hsrc)
	...
	//~CARN: for renaming mobs (updates their name, real_name, mind.name, their ID/PDA and datacore records).
		...
		var/new_mutantrace = input("Please choose a new mutantrace","Mutantrace",null) as null|anything in list("NONE","golem","lizard","slime","plant","shadow", "orange")
		...
		switch(Text)
			if("brute")	L.deal_overall_damage(amount, 0)
			if("fire")	L.deal_overall_damage(0, amount)
			if("toxin")	L.deal_damage(amount, TOX)
			if("oxygen")L.deal_damage(amount, OXY)
			if("brain")	L.deal_damage(amount, BRAIN)
			if("clone")	L.deal_damage(amount, CLONE)
			else
