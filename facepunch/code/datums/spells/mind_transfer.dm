...
*/
/obj/effect/proc_holder/spell/targeted/mind_transfer/cast(list/targets,mob/living/user = usr)
	if(!targets.len)
		user << "No mind found."
		return
			...
			for(var/j=checked_spells.len,(j>0&&checked_spells.len),j--)//While the spell list to check is greater than zero and has spells in it, run this proc.
				if(prob(base_spell_loss_chance))
					...
					spawn(msg_wait)
						...
						victim.unlock_achievement("You're a Wizard Harry?", announce = 0)
	...
	caster.deal_damage(paralysis_amount_caster, PARALYZE)
	victim.deal_damage(paralysis_amount_victim, PARALYZE)
