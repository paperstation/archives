...
/datum/teleport
	...
	proc/setTeleatom(atom/movable/ateleatom)
		if((istype(ateleatom, /obj/effect) && !istype(ateleatom, /obj/effect/critter))&& !istype(ateleatom, /obj/effect/dummy/chameleon))
			del(ateleatom)
			return 0
		if(istype(ateleatom))
			teleatom = ateleatom
...
/datum/teleport/instant/science
	...
	teleportChecks()
		...
		if(istype(teleatom, /obj/structure/stool/bed))//This is bad and more or less just deals with the janitor cart
			var/obj/structure/stool/bed/telebed = teleatom
			if(telebed.buckled_mob)
				buckledmob = telebed.buckled_mob
				if(!isemptylist(buckledmob.search_contents_for(/obj/item/weapon/disk/nuclear)))
					if(istype(buckledmob, /mob/living))
						var/mob/living/MM = buckledmob
						MM.visible_message("\red <B>The [MM] bounces off of the portal!</B>","\red Something you are carrying seems to be unable to pass through the portal. Better drop it if you want to go through.")
					else
						teleatom.visible_message("\red <B>The [teleatom] bounces off of the portal!</B>")
					return 0
		...
		if(destination.z > 5) //Away mission z-levels
			return 0
		return 1