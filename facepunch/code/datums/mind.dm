...
/*
			...
			if (ticker.mode.config_tag=="samurai")
				text = uppertext(text)
			text = "<i><b>[text]</b></i>: "
			if (src in ticker.mode.samurai)
				text += "<b>YES</b>|<a href='?src=\ref[src];samurai=clear'>no</a>"
				text += "<br><a href='?src=\ref[src];samurai=lair'>To lair</a>, <a href='?src=\ref[src];common=undress'>undress</a>, <a href='?src=\ref[src];wizard=dressup'>dress up</a>, <a href='?src=\ref[src];wizard=name'>let choose name</a>."
				if (objectives.len==0)
					text += "<br>Objectives are empty! <a href='?src=\ref[src];samurai=autoobjectives'>Randomize!</a>"
			else
				text += "<a href='?src=\ref[src];samurai=samurai'>yes</a>|<b>NO</b>"
			sections["samurai"] = text
*/
			/** CHANGELING ***/
			text = "changeling"
...
//				text += "<br>All the changelings are dead! Restart in [round((changeling.TIME_TO_GET_REVIVED-(world.time-changeling.changelingdeathtime))/10)] seconds."
	...
	Topic(href, href_list)
		...
		if (href_list["role_edit"])
			...
			var/new_obj_type = input("Select objective type:", "Objective type", def_value) as null|anything in list("assassinate", "debrain", "protect", "prevent", "hijack", "escape", "survive", "steal", "download", "nuclear", "capture", "absorb", "sword", "soul", "custom")
			...
			switch (new_obj_type)
				...
				if ("soul")
					new_objective = new /datum/objective/soul
					new_objective.owner = src

				if ("sword")
					new_objective = new /datum/objective/sword
					new_objective.owner = src
...
/*
		else if (href_list["samurai"])
			switch(href_list["samurai"])
				if("clear")
					if(src in ticker.mode.samurai)
						ticker.mode.samurai -= src
						special_role = null
						current.spellremove(current, config.feature_object_spell_system? "object":"verb")
						current << "\red <FONT size = 3><B>You have been brainwashed! You are no longer a samurai!</B></FONT>"
						log_admin("[key_name_admin(usr)] has de-samurai'ed [current].")
				if("samurai")
					if(!(src in ticker.mode.samurai))
						ticker.mode.samurai += src
						special_role = "samurai"
						//ticker.mode.learn_basic_spells(current)
						current << "<B>\red You are the Space samurai!</B>"
						log_admin("[key_name_admin(usr)] has samurai'ed [current].")
			//	if("lair")
			//		current.loc = pick(samuraitart)
				if("dressup")
					ticker.mode.equip_samurai(current)
		//		if("autoobjectives")
		//			ticker.mode.forge_samurai_objectives(src)
		//			usr << "\blue The objectives for samurai [key] have been generated. You can edit them and anounce manually."
*/
