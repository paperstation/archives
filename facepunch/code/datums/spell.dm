...
/obj/effect/proc_holder/spell/proc/adjust_var(mob/living/target = usr, type, amount) //handles the adjustment of the var when the spell is used. has some hardcoded types
	switch(type)
		if("bruteloss")
			target.deal_damage(amount, BRUTE)
		if("fireloss")
			target.deal_damage(amount, BURN)
		if("toxloss")
			target.deal_damage(amount, TOX)
		if("oxyloss")
			target.deal_damage(amount, OXY)
		if("weakened")
			target.deal_damage(amount, WEAKEN)
		if("paralysis")
			target.deal_damage(amount, PARALYZE)
		//else
