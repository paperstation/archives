...
/datum/disease/brainrot/stage_act() //Removed toxloss because damaging diseases are pretty horrible. Last round it killed the entire station because the cure didn't work -- Urist
	...
	switch(stage)
		if(2)
			...
			if(prob(5))
				affected_mob.brain_damage += 1
		if(3)
			if(prob(2))
				affected_mob.emote("stare")
			if(prob(2))
				affected_mob.emote("drool")
			if(prob(10) && affected_mob.brain_damage <=98)//shouldn't retard you to death now
				affected_mob.brain_damage += 2
				if(prob(2))
		...
		if(4)
			...
			if(prob(2))
				affected_mob.emote("drool")
			if(prob(15) && affected_mob.brain_damage <=98) //shouldn't retard you to death now
				affected_mob.brain_damage += 3
				if(prob(2))
			...
			if(prob(3))
				...
				affected_mob.deal_damage(8, PARALYZE)
				if(prob(1))
