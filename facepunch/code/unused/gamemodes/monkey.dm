...
/datum/game_mode/monkey/declare_completion()
	...
	if (monkeywin)
		...
		text += "<br><FONT size=3 color=red><B>The monkeys have won! Humanity is doomed!</B></FONT>"
		...
		text+= "<br><FONT size=3 color=red><B>The Research Staff has stopped the monkey invasion!</B></FONT>"
	..()
	outputstats += text
	return 1
	...
	for(var/mob/living/carbon/monkey/monkey_player in mob_list)
		for(var/datum/disease/D in monkey_player.viruses)
			if (istype(D, /datum/disease/jungle_fever) && monkey_player.ckey)
				outputstats += "<br><br><B>[monkey_player.ckey] was played infested [monkey_player]. [monkey_player.stat == 2 ? "(DEAD)" : ""]</B>"
	return 1
