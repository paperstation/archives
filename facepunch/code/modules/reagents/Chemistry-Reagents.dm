	...
	reagent
		...
		proc
			reaction_mob(var/mob/M, var/method=TOUCH, var/volume) //By default we have a chance to transfer some
				...
				var/datum/reagent/self = src//Why is this even here
				...
				if(!istype(self.holder.my_atom, /obj/effect/effect/chem_smoke))
					...
					if(method == TOUCH)
						var/block  = 0
						var/bioarmor = 0//This system is odd and should likely be redone but I am not going over the entire thing atm to figure out what its doing

						if(istype(M, /mob/living/carbon/human))
							var/mob/living/carbon/human/H = M
							if(H.wear_suit)
								bioarmor = H.wear_suit.armor[BIO]
							if(H.head)
								if(bioarmor < H.head.armor[BIO])
									bioarmor = H.head.armor[BIO]
						bioarmor *= 100
						if(prob(bioarmor))
							block = 1

						if(prob(100-bioarmor) && !block)
							if(M.reagents)
		...
		slimejelly
			...
			on_mob_life(var/mob/living/M as mob)
				if(prob(10))
					...
					M.deal_damage(rand(20,60)*REM, TOX)
				else if(prob(40))
					M.deal_damage(-5*REM, TOX)
					M.deal_overall_damage(-5*REM, -5*REM)
				..()
				return
...
*/
			...
			reaction_turf(var/turf/simulated/T, var/volume)
				...
				if(volume >= 3)
					...
					if(alt_water)
						...
						T.wet_overlay = image('icons/effects/water.dmi',T,"wet_floor_alt")
					T.overlays += T.wet_overlay
		...
		cyanide
			...
			on_mob_life(var/mob/living/M as mob)
				if(!M) M = holder.my_atom
				M.deal_damage(3*REM, TOX)
				M.deal_damage(3*REM, OXY)
				M.sleeping += 1
				..()
				return
			...
			on_mob_life(var/mob/living/M as mob)
				...
				switch(data)
					...
					if(25 to INFINITY)
						M.sleeping += 1
						M.oxy_damage = 0
						M.deal_damage(-50, WEAKEN)
						M.deal_damage(-50, PARALYZE)
						M.dizziness = 0
						M.drowsyness = 0
						M.stuttering = 0
						M.confused = 0
		...
		serotrotium
			...
			reagent_state = LIQUID
			color = "#C7FFFF" // rgb: 20, 20, 40

			on_mob_life(var/mob/living/carbon/M as mob) //hangovercentral
				if(holder.has_reagent("mindbreaker"))
		...
		electrozene
			name = "Electrozene"
			id = "electrozene"
			description = "A chemical compound that prevents the body from being fatally shocked."
			reagent_state = LIQUID
			color = "#C7FFFF" // rgb: 20, 20, 40
				...
				if(method == TOUCH)
					...
					if(!M.unacidable)
						if(prob(15) && istype(M, /mob/living/carbon/human) && volume >= 30)
							M.deal_overall_damage(0, 25)
							M.status_flags |= DISFIGURED
							M.emote("scream")
						else
							M.deal_overall_damage(0, min(15, volume * 2))// uses min() and volume to make sure they aren't being sprayed in trace amounts (1 unit != insta rape) -- Doohl
				else
					if(!M.unacidable)
						M.deal_overall_damage(0,min(15, volume * 2))
			...
			reaction_mob(var/mob/living/M, var/method=TOUCH, var/volume)
				...
				if(method == TOUCH)
					if(ishuman(M))
						...
						if(!H.unacidable)
							M.deal_damage(min(15, volume * 4), BURN)
							H.emote("scream")
					else if(ismonkey(M))
						var/mob/living/carbon/monkey/MK = M
						...
						if(!MK.unacidable)
							MK.deal_damage(min(15, volume * 4), BURN) // same deal as sulphuric acid
				else
					if(!M.unacidable)
						if(ishuman(M))
							M.deal_damage(15, TOX, null, "head")
							M.emote("scream")
							M.status_flags |= DISFIGURED
						else
							M.deal_damage(min(15, volume * 4), BURN)
...
/*
		...
		tricordrazine
			...
			reagent_state = LIQUID
			color = "#1FEC1F" // rgb: 200, 165, 220

			on_mob_life(var/mob/living/M as mob)
				if(M.stat == 2.0)
					return
				if(!M) M = holder.my_atom
				if(M.oxy_damage && prob(80)) M.deal_damage(-1*REM, OXY)
				if(M.get_brute_loss() && prob(80)) M.deal_overall_damage(-1*REM,0)
				if(M.get_fire_loss() && prob(80)) M.deal_overall_damage(0,-1*REM)
				if(M.tox_damage && prob(80)) M.deal_damage(-1*REM, TOX)
				..()
				return


		hallucinine //Heals hallucations
			name = "Hallucinine"
			id = "hallucinine"
			description = "Its not even real; once you use this at least."
			reagent_state = LIQUID
			color = "#5E00FF" // rgb: 200, 165, 220
		...
		adminordrazine //An OP chemical for admins
			...
			on_mob_life(var/mob/living/carbon/M as mob)
				...
				M.oxy_damage = 0
				M.radiation = 0
				M.deal_overall_damage(-100,-100)
				M.deal_damage(-100, TOX)
				if(holder.has_reagent("toxin"))
		...
		synaptizine
			...
			on_mob_life(var/mob/living/M as mob)
				...
				M.deal_damage(-1, WEAKEN)
				M.deal_damage(-1, PARALYZE)
				if(holder.has_reagent("mindbreaker"))
					holder.remove_reagent("mindbreaker", 5)
				M.hallucination = max(0, M.hallucination - 10)
				if(prob(60))	M.deal_damage(1, TOX)
				..()
				return
		...
		impedrezene
			...
			on_mob_life(var/mob/living/M as mob)
				...
				if(prob(80)) M.brain_damage += 1*REM
...
//				M.sdisabilities &= ~1		Replaced by eye surgery
		...
		cryoxadone
			...
			on_mob_life(var/mob/living/M as mob)
				...
				if(M.bodytemperature < 170)
					M.deal_damage(-3, CLONE)
					M.deal_damage(-3, OXY)
					M.deal_overall_damage(-3,-3)
					M.deal_damage(-3, TOX)
				..()
				return
		...
		clonexadone
			...
			reagent_state = LIQUID
			color = "#FFDF04" // rgb: 200, 165, 220

			on_mob_life(var/mob/living/M as mob)
				if(!M) M = holder.my_atom
				if(M.bodytemperature < 170)
					M.deal_damage(-6, CLONE)
					M.deal_damage(-6, OXY)
					M.deal_overall_damage(-6,-6)
					M.deal_damage(-6, TOX)
				..()
				return
		...
		zombiepowder
			...
			on_mob_life(var/mob/living/carbon/M as mob)
				...
				M.deal_damage(0.5*REM, OXY)
				M.deal_damage(0.5*REM, TOX)
				M.deal_damage(10, WEAKEN)
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		inspectionites
			name = "Inspectionites"
			id = "inspectionites"
			description = "Shit son, theres going to be a rump roast."
			reagent_state = LIQUID
			color = "#535E66" // rgb: 83, 94, 102
			...
			reaction_mob(var/mob/M, var/method=TOUCH, var/volume)
				src = null
				if( (prob(10) && method==TOUCH) || method==INGEST)
					M.contract_disease(new /datum/disease/assinspection(0),1)

		inspectionitesplacebo
			name = "inspectionitesplacebo"
			id = "inspectionitesplacebo"
			description = "ASS-P"
			reagent_state = LIQUID
			color = "#535E66" // rgb: 83, 94, 102
			...
			reaction_mob(var/mob/M, var/method=TOUCH, var/volume)
				src = null
				if( (prob(10) && method==TOUCH) || method==INGEST)
					M.contract_disease(new /datum/disease/assinspectionplacebo(0),1)
...
// 	condiments, additives, and such go.
		nutriment
			...
			on_mob_life(var/mob/living/M as mob)
				if(!M) M = holder.my_atom
				if(prob(50)) M.deal_overall_damage(-1,0)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		orangejuice
			...
			on_mob_life(var/mob/living/M as mob)
				M.nutrition += nutriment_factor
				if(!M) M = holder.my_atom
				if(M.oxy_damage && prob(30)) M.deal_damage(-1, OXY)
				M.nutrition++
				..()
				return
		...
		tomatojuice
			...
			on_mob_life(var/mob/living/M as mob)
				M.nutrition += nutriment_factor
				if(!M) M = holder.my_atom
				if(M.get_fire_loss() && prob(20)) M.deal_damage(-1, BURN)
				M.nutrition++
				..()
				return
		...
		limejuice
			...
			on_mob_life(var/mob/living/M as mob)
				M.nutrition += nutriment_factor
				if(!M) M = holder.my_atom
				if(prob(20)) M.deal_damage(-1*REM, TOX)
				M.nutrition++
				..()
				return
		...
		milk
			...
			on_mob_life(var/mob/living/M as mob)
				if(!M) M = holder.my_atom
				if(prob(20)) M.deal_overall_damage(-2,0)
				if(holder.has_reagent("capsaicin"))
		...
		soymilk
			...
			on_mob_life(var/mob/living/M as mob)
				if(!M) M = holder.my_atom
				if(prob(20)) M.deal_overall_damage(-1,0)
				M.nutrition++
				..()
				return
		...
		tea
			...
			on_mob_life(var/mob/living/M as mob)
				...
				if(prob(20)) M.deal_damage(-1, TOX)
				if(M.bodytemperature < 310)  //310 is the normal bodytemp. 310.055
...
/////////////////////////////////////////////////////////////////cocktail entities//////////////////////////////////////////////
		...
		bilk
			...
			on_mob_life(var/mob/living/M as mob)
				if(prob(10)) M.deal_overall_damage(-1,0)
				M.nutrition += 2
				if(!data) data = 1
				data++
				M.make_dizzy(3)
		...
		doctor_delight
			...
			on_mob_life(var/mob/living/M as mob)
				if(!M) M = holder.my_atom
				if(prob(80)) M.deal_damage(-2, OXY)
				if(prob(80)) M.deal_overall_damage(-2, -2)
				if(prob(80)) M.deal_damage(-2, TOX)
		...
		malley
			name = "O'Malley Ale"
			id = "malley"
			description = "Sure enough you are drunk.."
			reagent_state = LIQUID
			color = "#664300" // rgb: 102, 67, 0
		...
		slaughterwine
			name = "Slaughter Wine"
			id = "slaughterwine"
			description = "Why is the world spinning?"
			reagent_state = LIQUID
			color = "#C80000" // rgb: 200, 0, 0
		...
		winemeh
			name = "Wine"
			id = "winemeh"
			description = "Pretty average tasting."
			reagent_state = LIQUID
			color = "#C80000" // rgb: 200, 0, 0
		...
		malanthawine
			name = "Malantha Wine"
			id = "malanthawine"
			description = "Tastes like radiation?"
			reagent_state = LIQUID
			color = "#C80000" // rgb: 200, 0, 0
		...
		scooterwine
			name = "Scooterica Wine"
			id = "scootericaawine"
			description = "Why does it taste like broken?"
			reagent_state = LIQUID
			color = "#C80000" // rgb: 200, 0, 0
		...
		luiwine
			name = "Lui Wine"
			id = "luiwine"
			description = "Isn't really filling."
			reagent_state = LIQUID
			color = "#C80000" // rgb: 200, 0, 0
		...
		winehshitholyshit
			name = "Wine"
			id = "wineohshitholyshit"
			description = "My life is complete."
			reagent_state = LIQUID
			color = "#C80000" // rgb: 200, 0, 0
			...
			on_mob_life(var/mob/living/M as mob)
				...
				data++
				M.make_dizzy(10)
				M.jitteriness = max(M.jitteriness-55,0)
				M.nutrition += 20
				if(data >= 25)
					if (!M.stuttering) M.stuttering = 1
					M.stuttering += 20
		...
		foospunch
			name = "Foospunchia Wine"
			id = "foospunch"
			description = "God like in quality 10 star review."
			reagent_state = LIQUID
			color = "#C80000" // rgb: 200, 0, 0
		...
		xenon
			name = "Xenon"
			id = "xenon"
			description = "A general anesthesia chemical."
			reagent_state = LIQUID
			color = "#019CA8" // rgb: 0, 156, 168
		...
		indium
			...
			reagent_state = LIQUID
			color = "#01d342" // rgb: 0, 156, 168

		batter
			name = "Cookie Batter"
			id = "batter"
			description = "Makes yummy christmas cookies."
			reagent_state = LIQUID
			color = "#000051" // rgb: 0, 156, 168
...
//Photo lab stuff
		...
		developer
			name = "Developer"
			id = "developer"
			description = "Put your photos in this first."
			reagent_state = SOLID
			color = "#D0D0D0"
		...
		stopbath
			name = "Stop Bath"
			id = "stopbath"
			description = "Put your photos in this second."
			reagent_state = SOLID
			color = "#D0D0D0"
		...
		fixer
			name = "Fixer"
			id = "fixer"
			description = "Put your photos in this third."
			reagent_state = SOLID
			color = "#000051" // rgb: 0, 156, 168
