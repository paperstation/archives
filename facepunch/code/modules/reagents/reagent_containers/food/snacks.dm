...
/obj/item/weapon/reagent_containers/food/snacks/attack(mob/M as mob, mob/user as mob, def_zone)
	...
	if(istype(M, /mob/living/carbon))
		...
		if(M == user)								//If you're eating it yourself.
			var/fullness = L.nutrition + (L.reagents.get_reagent_amount("nutriment") * 25)
			if(fullness <= 50)
				L << "\red You hungrily chew out a piece of [src] and gobble it!"
			if(fullness > 50 && fullness <= 150)
				L << "\blue You hungrily begin to eat [src]."
			if(fullness > 150 && fullness <= 350)
				L << "\blue You take a bite of [src]."
			if(fullness > 350 && fullness <= 550)
				L << "\blue You unwillingly chew a bit of [src]."
			if(fullness > (550 * (1 + L.overeatduration / 2000)))	// The more you eat - the more you can eat
				L << "\red You cannot force any more of [src] to go down your throat."
				return 0
		else
			...
			var/fullness = L.nutrition + (L.reagents.get_reagent_amount("nutriment") * 25)
			if (fullness <= (550 * (1 + L.overeatduration / 1000)))
			...
			if(!do_mob(user, L)) return

			L.attack_log += text("\[[time_stamp()]\] <font color='orange'>Has been fed [src.name] by [user.name] ([user.ckey]) Reagents: [reagentlist(src)]</font>")
			user.attack_log += text("\[[time_stamp()]\] <font color='red'>Fed [src.name] by [M.name] ([M.ckey]) Reagents: [reagentlist(src)]</font>")

			log_attack("<font color='red'>[user.name] ([user.ckey]) fed [L.name] ([L.ckey]) with [src.name] (INTENT: [uppertext(user.a_intent)])</font>")

			for(var/mob/O in viewers(world.view, user))
				O.show_message("\red [user] feeds [L] [src].", 1)
...
/obj/item/weapon/reagent_containers/food/snacks/cookie/christmas
	name = "Christmas Cookie"
	desc = "Santa might want this"
	icon_state = "COOKIE!!!"
	New()
		..()
		reagents.add_reagent("nutriment", 5)
...
obj/item/weapon/reagent_containers/food/snacks/ianburger
	name = "Ian Burger"
	desc = "You'll just scream after eating this poor cuisine."
	icon_state = "ianburger"
	New()
		..()
		reagents.add_reagent("nutriment", 6)
...
/obj/item/weapon/reagent_containers/food/snacks/superbiteburger/special
	New()
		..()
		reagents.add_reagent("chloralhydrate", 40)
		bitesize = 15
...
/obj/item/weapon/reagent_containers/food/snacks/sliceable/pizza
	...
	attackby( obj/item/I as obj, mob/user as mob)
		if(istype(I,/obj/item/toy/spinningtoy))
			if(!istype(src.loc, /turf))
				user << "\blue You feel like you should put the pizza down first."
				return
			user.drop_item()
			del(I)
			user.visible_message("\blue [user] places the [I] into the [src].")
			var/obj/effect/critter/pizza/pizza = new/obj/effect/critter/pizza(src.loc)
			pizza.name = src.name
			pizza.pizza_type = src.type
			del(src)
			return
...
/obj/item/weapon/reagent_containers/food/snacks/affinity
	name = "Affinity +100"
	desc = "A glowing blue ball... maybe I could eat it."
	icon = 'icons/obj/warframe_objs.dmi'
	icon_state = "wf_affinity"
	New()
		..()
		reagents.add_reagent("nutriment", 10)
		bitesize = 1