/obj/structure/darts
	name = "Dart Board"
	desc = "Get drunk, get krunked in darts."
...
/obj/structure/potofgold
	name = "Pot of Gold"
	desc = "You might get something!"
	icon = 'icons/obj/scooterstuff.dmi'
	icon_state = "potofgold"
	density = 1
	var/percent = 1
	var/list/mob/living/C = list()
	attack_hand(var/mob/user as mob)
		...
		if(prob(percent))
			if(prob(25))
				...
				user << "\red You search around the [name] and obtain a point!"
				spawn(30)
					user << "\red And an achievement!"
					user.unlock_achievement("Potluck")
			else
				user << "\red You search around the [name] but find nothing of interest!"
		else
			user << "\red You search around the [name] but find nothing of interest!"