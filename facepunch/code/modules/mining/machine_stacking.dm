...
/obj/machinery/mineral/stacking_machine/New()
	...
	spawn( 5 )
		...
		for (var/dir in cardinal)
			...
			if(src.output) break
		//processing_objects.Add(src) YOU ARE A MACHINE WHY THE FUCK ARE YOU ON THIS PROCESS LIST
		return
	return
...
/obj/machinery/mineral/scrap_storage
	name = "scrap storage"
	icon = 'icons/obj/machines/mining_machines.dmi'
	icon_state = "stacker"
	density = 1
	anchored = 1.0
	var/obj/machinery/mineral/stacking_unit_console/scrap/CONSOLE
	var/obj/machinery/mineral/input = null
	var/obj/machinery/mineral/output = null
	var/glass = 0
	var/waste = 0
...
/obj/machinery/mineral/scrap_storage/New()
	..()
	spawn( 5 )
		for (var/dir in cardinal)
			src.input = locate(/obj/machinery/mineral/input, get_step(src, dir))
...
/obj/machinery/mineral/scrap_storage/process()
	if (locate(/obj/item, input.loc))
		var/obj/item/O
		var/i
		for (i = 0; i<10; i++)
			O = locate(/obj/item/scrap, input.loc)
			if (O)
				metal += O.m_amt
				glass += O.g_amt
				waste += O.w_amt
