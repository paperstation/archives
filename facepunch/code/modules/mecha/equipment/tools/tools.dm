	...
	action(atom/target)
		...
		if(istype(target,/obj))
			...
			if(chassis.occupant.a_intent == "hurt")
				M.deal_overall_damage(dam_force,0)
...
/obj/item/mecha_parts/mecha_equipment/antiproj_armor_booster
	...
	proc/dynbulletdamage(var/obj/item/projectile/Proj)
		...
		if(prob(chassis.deflect_chance*deflect_coeff))
			...
			chassis.take_damage(round(Proj.force*src.damage_coeff))
	...
	proc/dynhitby(atom/movable/A)
		...
		if(prob(chassis.deflect_chance*deflect_coeff) || istype(A, /mob/living) || istype(A, /obj/item/mecha_parts/mecha_tracking))
			...
			var/obj/O = A
			if(O.force)
				chassis.take_damage(round(O.force*damage_coeff))
				chassis.check_for_internal_damage(list(MECHA_INT_TEMP_CONTROL,MECHA_INT_TANK_BREACH,MECHA_INT_CONTROL_LOST))
		set_ready_state(0)
		chassis.use_power(energy_drain)
		do_after_cooldown()
...
/datum/global_iterator/mecha_generator/nuclear
	...
	process(var/obj/item/mecha_parts/mecha_equipment/generator/nuclear/EG)
		if(..())
			for(var/mob/living/M in view(EG.chassis))
				M.deal_damage((EG.rad_per_cycle*3), IRRADIATE, IRRADIATE)
		return 1
