...
/obj/item/weapon/paper
	...
	var/list/stamped
	var/parse = 0//If this is set to 1 when this paper is created it should call parsepencode.  This allows us to input papers using the in game format
...
/obj/item/weapon/paper/New()
	...
	spawn(2)
		if(parse)
			info = parsepencode(info, null, null, 0)
		update_icon()
		updateinfolinks()
...
/obj/item/weapon/paper/crumpled/assx
	icon_state = "scrap_bloodied"
	info = "Don't worry Doctor Kevin; the security systems we installed will be pretty effect. The fields guarding the way into the lab will ignite the plasma inside the room. The scientists have their own way in via their modified jumpsuits; they just need to jump into the singularity. Of course everyone else has to go through the lab. If someone touches the fields; the plasma ignites and I bet you understand what will happen. Also if they try to bomb it; there is a 63.1 per cent chance that it will blow the disposal up and or ignite the plasma. Also the Scientists have been trained in multiple forms of martial arts; some are scared others will hop on the fighting train and kill intruders. <BR>\n\tSigned, Doctor Dan"


/obj/item/weapon/paper/ert
	name = "paper- 'Standard Operating Procedure'"
	info = "ERT - Standard Operating Procedure<BR>1. Gather your gear and move supplies onto the shuttle<BR>\n\t2. Put on spaces suits if the situation requires (Optional)<BR>3. Get on the shuttle and go to the Station. On Station SOR:<BR>Pick the Vacant Office; the old Assembly line; or the Construction area as a base of operations; you have all access. If you are unable to use one of these three places; use the next best or secure place. From there; install the Communications Console provided on Central Command. :<BR>Accomplish your mission objectives to their fullest.<BR><BR>Do not allow people onto Central Command with your ship. Contact Central Command as soon as the Communications Console is setup for them to recall the ship.</BR></BR>"



/obj/item/weapon/paper/dnd_rules
	name = "paper- 'SDnD Rules'"
	parse = 1
	info = "\
	\[small]Page 1\[/small]\[br]\[br]\
	\[large]\[center] Guide to space DnD \[/center]\[/large]\
	\[br]\[br]Requirements\[br]\
	\[list]\[*] 1 Dungeon master\[br]\
	\[*] 2-4 players + Character sheets\[br]\
	\[*] Dice set\[br]\[*] 1 Guide to Dungeons and Dragons\[br]\
	\[*] 1 pen\[br]\
	\[br]\[br]Welcome to Space DnD. First thing you will need is X amount of character sheets. X being the amount of players you have. Pass pens out, and have them fill out their character sheets. All players start with these stats:\
	\[br]\[br]STM : 10 \[br]\[small]Stamina determines what your maximum health can be. 1 point for ever point of stamina. You may be granted more than one stamina point at once\[/small]\[br]\
	\[br]INT : 3\[br]\[small]Intelligence decides the chance to hit you have with magic abilities and weapons. You must roll your intelligence or below on a D6 to hit with a magic ability or weapon\[/small]\[br]\
	\[br]STR : 3\[br]\[small]Strength is similar to intelligence but instead of magic weapons and abilities, it affects physical attacks. Strength also determines the order of attack.\[/small]\[br]\
	\[br]\[center]Setting up a game\[/center]\[br]\[br]\[small]Firstly you must ask each player what item he or she would like to start with.\[br]\[br]\[list]\[*]Staff of healing - Heals target for 2 health points\[br]\[*]Sword - Damages target for 3 health points\[br]\[*]Staff of Attack - Damages the target by 2 damage this round, and 1 damage every turn on the users turn for two turns.\[/list]\[/small]\[br]\
	\[small]Both Staves use INT to determine hits.\[/small]\[br]\[br]\
	Now that the players have chosen what item they will use, each of them may choose 1 special ability, which may only be used once per combat session, consuming your turn.\[br]\[br]\[list]\[*]Firebolt - Deals 2 damage, and causes all attacks against this target to deal 1 additional damage until combat ends\[br]\[*]Cleave - Deals 4 damage (This ability always hits)\[br]\[*]Greater Heal - Roll a D6. You may distribute this amount of health amongst your party.\[/list]\[br]\[br]\
	\[center]Being a good GM\[/center]\[br]\[br]\
	\[list]\[*]Remember to roll for hits\[br]\[*]You are in charge of deciding when people level up. Don't be too generous, but keeping them at 3,3,10 a whole game is not nice!\[br]\[*]It's a roleplaying game, keep them in character, but have fun!\[br]\[*]You can add items, enemies, scenarios, events, and anything else you want. Get creative!\[br]\[br]\
	Well, it's out of my hands now. Good luck, and have fun!"

/obj/item/weapon/paper/dnd_charactersheet
	name = "paper- 'SDnD Character Sheet'"
	parse = 1
	info = "\
	\[center]Character Sheet\[/center]\[br]\[br]\
	\[small]Character Name : \[field] \[br]\[br]\
	Played by : \[field] \[br]\[br]\
	Weapon choice : \[field] \[br]\[br]\
	Ability choice : \[field] \[br]\[br]\
	INT : 3 + \[field]\[br]\
	STR : 3 + \[field]\[br]\
	STM : 10 + \[field]\[br]\[br]\
	Health : Current \[field] / Maximum \[field]"
