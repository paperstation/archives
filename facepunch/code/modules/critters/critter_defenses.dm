...
/obj/effect/critter
		...
		if (user.a_intent == "hurt")
			...
			if(istype(user, /mob/living/carbon/human))
				for(var/mob/O in viewers(src, null))
					O.show_message("\red <B>[user] has punched the [src]!</B>", 1)
				...
				for(var/mob/O in viewers(src, null))
					O.show_message("\red <B>[user] slashes the [src]!</B>", 1)
				playsound(src.loc, 'sound/weapons/slice.ogg', 25, 1, -1)

			else
				for(var/mob/O in viewers(src, null))
					O.show_message("\red <B>[user] bites the [src]!</B>", 1)

			if(src.defensive)	Target_Attacker(user)
		else
			for(var/mob/O in viewers(src, null))
				O.show_message("\blue [user] touches the [src]!", 1)
	...
	proc/TakeDamage(var/damage = 0)
		var/tempdamage = (damage-damage_resistance)
		if(tempdamage > 0)
	...
	damage(var/amount, var/use_armor = 1)
		TakeDamage(amount)
