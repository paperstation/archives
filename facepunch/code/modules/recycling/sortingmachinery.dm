...
/obj/machinery/disposal/deliveryChute/wildride
	name = "Chute"
	desc = "Mr Bones Wild Ride!"
	density = 1
	damage_resistance = -1
	icon_state = "intake"
	var/skeleton = 0//Convert to skeleton or not

	attackby(var/obj/item/I, var/mob/user)
		return//Nope

	Bumped(var/atom/movable/AM) //Go straight into the chute
		if(istype(AM, /mob/living/carbon/human) && skeleton)
