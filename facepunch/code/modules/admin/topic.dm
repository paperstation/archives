...
/datum/admins/Topic(href, href_list)
	...
	if(!check_rights(0))
		message_admins("[usr.key] admin permissions check failure!")
		log_admin("[key_name(usr)] admin permissions check failure.")
		return
		...
		switch(href_list["makeAntag"])
			...
			if("10")
				...
				if(!src.makeDeathsquad())
					usr << "\red Unfortunatly there were no candidates available"
			if("11")
				log_admin("[key_name(usr)] has spawned a borg death squad.")
//				if(!src.makeBorgDS())
//					usr << "\red Unfortunatly there were no candidates available"
...
*/
	...
	else if(href_list["robust"])
		if(!check_rights(R_ADMIN))	return

		var/mob/M = locate(href_list["robust"])
		if(!ismob(M))
			usr << "this can be only used on instances of type /mob"
			return

		log_admin("[key_name(usr)] has granted [key_name(M)] the robust achievement")
		message_admins("\blue [key_name_admin(usr)] has granted [key_name_admin(M)] the robust achievement", 1)
		M.unlock_achievement("Professional Toolbox Champion")


	else if(href_list["point"])
		if(!check_rights(R_ADMIN))	return
		var/ckey = href_list["ckey"]
		...
		if(istext(ckey))
			...
			if(C)
				C.givepoint(num)
				log_admin("[key_name(usr)] has granted [key_name(C)] [num] point(s)")
				message_admins("\blue [key_name_admin(usr)] has given [key_name_admin(C)] [num] point(s)", 1)
...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
		...
		if(isliving(M))
			...
			health_description += "<BR>Oxy: [L.oxy_damage] - Tox: [L.tox_damage] - Fire: [L.get_fire_loss()] - Brute: [L.get_brute_loss()] - Clone: [L.clone_damage] - Brain: [L.brain_damage]"
		else
		...
		if(M.health == 1)
			...
			M.deal_damage(min(99, (M.health - 1)), BRUTE, PIERCE, "chest")
			M.deal_damage(20, WEAKEN)
			M.stuttering = 20
...
/*				if("shockwave")
									...
									if(F.lava)
										...
										for(var/obj/structure/O in F.contents)
											if(O.level > F.level && !istype(O, /obj/structure/window)) // Something to stand on and it isn't under the floor!
												...
												break
										if(!safe)
											L.deal_damage(damage, BURN, IMPACT)
			...
			if("eagles")//SCRAW
				...
				for(var/obj/machinery/door/airlock/W in world)
					if(W.z == 1 && !istype(get_area(W), /area/station/command/bridge) && !istype(get_area(W), /area/crew_quarters) && !istype(get_area(W), /area/station/security/prison))
						W.req_access = list()
...
//				message_admins("[key_name_admin(usr)] has triggered a battle to the death (only one)")
		...
		switch(href_list["secretscoder"])
			...
			if("maint_access_brig")
				...
				message_admins("maint sec only does not function anymore.")
			if("maint_access_engiebrig")
				message_admins("maint sec/eng only does not function anymore.")
			if("emergency_armory")
