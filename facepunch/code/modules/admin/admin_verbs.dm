...
//	/datum/admins/proc/show_traitor_panel,	/*interface which shows a mob's mind*/ -Removed due to rare practical use. Moved to debug verbs ~Errorage
	...
	/client/proc/cmd_admin_traitor_narrate,	/*send an message to somebody as a 'voice in their head'*/
...
//	/client/proc/cmd_manual_ban, //NOT DONE YET
...
var/list/admin_verbs_hideable = list(
	...
	/client/proc/cmd_admin_traitor_narrate,
...
/client/proc/secrets()
	...
	if(holder)
		holder.Secrets()
	mob.unlock_achievement("HAW HAW I AM TOUCH BUTTON")
	return
...
/client/proc/warn(warned_ckey)
	...
	if(++D.warns >= MAX_WARNS)					//uh ohhhh...you'reee iiiiin trouuuubble O:)
		...
		var/warns_left = MAX_WARNS - D.warns
		if(C)
			C << "<font color='red'><BIG><B>You have been formally warned by an administrator.</B></BIG></font>"
			message_admins("[key_name_admin(src)] has warned [key_name_admin(C)]. They have [warns_left] strikes remaining.")
		else
			message_admins("[key_name_admin(src)] has warned [warned_ckey] (DC). They have [warns_left] strikes remaining.")
...
/client/proc/adminrespawn()
	...
	set category = "Special Verbs"
	if (!holder)//Badplayers be badding
		return
	if(!ticker)
		usr << "<font color='red'>The game hasn't started yet!</font>"
		return
	if(alert("Are you sure you want to return to the lobby? This will disconnect you from your body and will count as ghosting.",,"Yes","No")!="Yes")
		return

	log_game("[usr.name]/[usr.key] returned to the lobby.")

	usr << "\blue <B>Don't abuse this feature please!</B>"

	var/mob/new_player/M = new /mob/new_player()
	if(!holder)
		log_game("Return to lobby for [usr.key] failed.")
		del(M)
		return
	M.key = key
	return