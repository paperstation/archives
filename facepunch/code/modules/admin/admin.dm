...
/proc/message_grief(var/msg)
	msg = "<span class=\"admin\"><span class=\"prefix\">GRIEF LOG:</span> <span class=\"message\">[msg]</span></span>"
	log_adminwarn(msg)
	for(var/client/C in admins)
		if(C.prefs.toggles & LOG_GRIEF)
			C << msg
		else
			return
...
/datum/admins/proc/show_player_panel(var/mob/M in mob_list)
	...
	if (M.client)
		...
		body += "<A href='?src=\ref[src];robust=\ref[M]'>Grant Robust</A> | "
		body += "<A href='?src=\ref[src];point=1;ckey=[M.ckey]'>Grant a Point</A> | "
	body += "<br>"
	body += "</body></html>"
...
/datum/admins/proc/Secrets()
	...
	if(check_rights(R_FUN,0))
		dat += {"
			...
			<A href='?src=\ref[src];secretsfun=ert'>Send in an ERT</A><BR>
...
/datum/admins/proc/announce()
	...
	if(!check_rights(0))	return
	var/list/choices = list("No", "Yes")
	var/choice = input("Would you like to make an admin announcement??") in choices
	switch(choice)
		if(null)
			return 0
		if("Yes")
...
/proc/is_special_character(mob/M as mob) // returns 1 for specail characters and 2 for heroes of gamemode
	...
	if(M.mind in ticker.mode.wizards)
		if (ticker.mode.config_tag == "wizard")
			return 2
		return 1
//	if(M.mind in ticker.mode.samurai)
//		if (ticker.mode.config_tag == "samurai")
//			return 2
//		return 1
...
proc/move_admin_shuttle()
	...
	if (admin_shuttle_location == 1)
		fromArea = locate(/area/shuttle/mysterious/station)
		toArea = locate(/area/shuttle/mysterious/centcom)
	else
		fromArea = locate(/area/shuttle/mysterious/centcom)
		toArea = locate(/area/shuttle/mysterious/station)
...
var/casino_location = 0 // 0 = centcom , 1 = station

proc/move_casino()
	var/area/fromArea
	var/area/toArea
	if (casino_location == 1)
		fromArea = locate(/area/shuttle/away/syndieattack/ride)
		toArea = locate(/area/shuttle/away/syndieattack/station)
	else
		fromArea = locate(/area/shuttle/away/syndieattack/station)
		toArea = locate(/area/shuttle/away/syndieattack/ride)
	fromArea.move_contents_to(toArea)
	if (casino_location)
		casino_location = 0
	else
...
var/elevator_location = 0

proc/move_elevator()
	var/area/fromArea
	var/area/toArea
	if (elevator_location ==1)
		fromArea = locate(/area/world6/elevatorfloor1)
		toArea = locate(/area/world6/elevatorcasino)
	else if(elevator_location == 2)
		fromArea = locate(/area/world6/elevatorcasino)
		toArea = locate(/area/world6/elevatormain)
	else if(elevator_location == 0)
		fromArea = locate(/area/world6/elevatormain)
		toArea = locate(/area/world6/elevatorfloor1)
	fromArea.move_contents_to(toArea)
	if (elevator_location == 0)
		elevator_location = 1
	else if(elevator_location == 1)
		elevator_location = 2
	else if(elevator_location == 2)
		elevator_location = 0
	return
