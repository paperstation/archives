...
/proc/AddBan(ckey, computerid, reason, bannedby, temp, minutes, address, date)
	var/date_string = time2text(world.realtime,"DD-MMM-YYYY")
	var/bantimestamp
	if (temp)
		UpdateTime()
	...
	if ( Banlist.dir.Find("[ckey][computerid]") )
		...
		Banlist["temp"] << temp
		Banlist["date"] << date_string
		if (temp)
			Banlist["minutes"] << bantimestamp
			notes_add(ckey, "Banned - [reason], Duration: [minutes] minutes.")
		else
			notes_add(ckey, "Banned - [reason], Duration: Permanent.")
	return 1
...
/datum/admins/proc/unbanpanel()
	...
	for (var/A in Banlist.dir)
		...
		dat += text("<tr><td><A href='?src=[ref];unbanf=[key][id]'>(U)</A><A href='?src=[ref];unbane=[key][id]'>(E)</A> Key: <B>[key]</B></td><td>ComputerID: <B>[id]</B></td><td>IP: <B>[ip]</B></td><td>Date Banned: <B>[date]</b></td><td> [expiry]</td><td>(By: [by])</td><td>(Reason: [reason])</td></tr>")
