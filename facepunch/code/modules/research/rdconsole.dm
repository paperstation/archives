...
/obj/machinery/computer/rdconsole/proc/check_mats(var/machine_type = 0, var/datum/design/D)//Type 0 is Proto type 1 is Imprinter
	if(!istype(D))
		return 0
	switch(machine_type)
		if(0)
			...
			for(var/M in D.materials)
				if(copytext(M, 1, 2) == "$")
					switch(M)
						if("$glass")
							if(D.materials[M] > linked_lathe.g_amount) return 0
						if("$metal")
							if(D.materials[M] > linked_lathe.m_amount) return 0
						if("$gold")
							if(D.materials[M] > linked_lathe.gold_amount) return 0
						if("$silver")
							if(D.materials[M] > linked_lathe.silver_amount) return 0
						if("$plasma")
							if(D.materials[M] > linked_lathe.plasma_amount) return 0
						if("$uranium")
							if(D.materials[M] > linked_lathe.uranium_amount) return 0
						if("$diamond")
							if(D.materials[M] > linked_lathe.diamond_amount) return 0
						if("$clown")
							if(D.materials[M] > linked_lathe.clown_amount) return 0
		...
		if(1)
			...
			for(var/M in D.materials)
				if(copytext(M, 1, 2) == "$")
					switch(M)
						if("$glass")
							if(D.materials[M] > linked_imprinter.g_amount) return 0
						if("$gold")
							if(D.materials[M] > linked_imprinter.gold_amount) return 0
						if("$diamond")
							if(D.materials[M] > linked_imprinter.diamond_amount) return 0
				else if(!linked_imprinter.reagents.has_reagent(M, D.materials[M]))
					return 0
			return 1
	return 0//No machine type
						...
						if(check_mats(0, being_built))
							...
							if(being_built.build_path)
								...
								if(being_built.locked)
									if(istype(new_item,/obj/item/weapon/gun))
										var/obj/item/weapon/gun/G = new_item
										G.locked = 1//Weapon locks
										G.loc = linked_lathe.loc//Drop it on the ground
									else
