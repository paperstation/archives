/obj/machinery/power/antimatter/control_unit
	...
	var/list/obj/machinery/antimatter/parts

	//Based upon the inputs
	var/antimatter = 0
	var/antimatter_max = 0
	var/matter = 0
	var/matter_max = 0

	var/core_strength = 0	//Cores add to this its how much can be mixed safely
	var/injection_matter = 0	//How much per reaction
	var/injection_antimatter = 0 //How much per raction

	var/stability = 100//health
	...
	var/power_cycle = 0//Current tick
	var/power_cycle_delay = 4//How many ticks till produce_power is called, can be edited
...
/*
	process()
		if(exploding)//Needs to be changed, the cores are what should explode
			explosion(get_turf(src),8,12,18,12,1,1)
			if(src) del(src)
