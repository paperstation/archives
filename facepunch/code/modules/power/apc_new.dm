...
/obj/machinery/power/apc
	...
	var/lighting_percent = 20
	var/equipment_percent = 30
	var/environ_percent = 5 //This is not zero because I thought it'd be cool if APCs reserved a tiny bit of power so engies can "rerouting emergency power cap'n"
...
/obj/machinery/power/apc/New(turf/loc, var/ndir, var/building=0)
	...
	//If we have the default name then rename us to the area name so we dont have to do this while mapping
	if(name == "area power controller")
...
/obj/machinery/power/proc/channelmodetext(on, percent)
	. = on ? "On(" : "Off("
	switch(percent)
		if(0)	. += "AlwaysOn"
		if(100)	. += "AlwaysOff"
...
/obj/machinery/power/apc/interact(mob/user)
	...
	if ( (get_dist(src, user) > 1 ))//Distance check
		if(!istype(user, /mob/living/silicon))
			user.unset_machine()
	...
	var/factor = SSpower.wait ? 10 / SSpower.wait : 1
	if((locked && !istype(user, /mob/living/silicon)) || (aidisabled && istype(user, /mob/living/silicon)) )//The screen is shown when you cant access it
		t += "<I>(Swipe ID card to unlock inteface.)</I><BR>"

		t += "<table width=100% border=0>"
		t += "<tr><td>Main breaker</td><td>[operating ? "On" : "Off"]</td></tr>"
		t += "<tr><td>External power</td><td>[main_status ? (main_status ==2 ? "<FONT COLOR=#004000>Good</FONT>" : "<FONT COLOR=#D09000>Low</FONT>") : "<FONT COLOR=#F00000>None</FONT>"]</td></tr>"
		t += "<tr><td>Power cell</td><td>[cell ? "[round(cell.percent())]% ([charging ? ( charging == 1 ? "Charging" : "Fully charged" ) : "Not charging"]) ([chargemode ? "Auto" : "Off"])" : "<FONT COLOR=red>Not connected.</FONT>"]</td></tr>"
		t += "<tr><td><BR>Power channels</td><td></td></tr>"
		t += "<tr><td>Equipment<BR></td><td>[add_lspace(round(lastused_equip*factor), 6)] W/s : <B>[channelmodetext(equipment, equipment_percent)]<br></td></tr>"
		t += "<tr><td>Lighting<BR></td><td>[add_lspace(round(lastused_light*factor), 6)] W/s : <B>[channelmodetext(lighting, lighting_percent)]<br></td></tr>"
		t += "<tr><td>Environmental<BR></td><td>[add_lspace(round(lastused_environ*factor), 6)] W/s : <B>[channelmodetext(environ, environ_percent)]<br></td></tr>"
		t += "<tr><td>Total Load</td><td>[round((lastused_light + lastused_equip + lastused_environ)*factor)] W/s</td></tr>"
		t += "<tr><td>Cover Lock</td><td>[coverlocked ? "Engaged" : "Disengaged"]</td></tr>"
		t += "</table>"

	else//Unlocked screen silicon see this unless AI is disabled
		//if (!istype(user, /mob/living/silicon))
		t += "<I>(Swipe ID card to lock interface.)</I><BR>"

		t += "<table width=100% border=0>"
		t += "<tr><td>Main breaker</td><td>[operating ? "<B>On</B> <A href='?src=\ref[src];breaker=1'>Off</A>" : "<A href='?src=\ref[src];breaker=1'>On</A> <B>Off</B>"]</td></tr>"
		t += "<tr><td>External power</td><td>[main_status ? (main_status ==2 ? "<FONT COLOR=#004000>Good</FONT>" : "<FONT COLOR=#D09000>Low</FONT>") : "<FONT COLOR=#F00000>None</FONT>"]</td></tr>"
		t += "<tr><td>Power cell</td><td>"
		if(cell)
			var/chargetext = charging ? ( charging == 1 ? "Charging" : "Fully charged" ) : "Not charging"
			var/modetext = chargemode ? "<A href='?src=\ref[src];cmode=1'>Off</A> <B>Auto</B>" : "<B>Off</B> <A href='?src=\ref[src];cmode=1'>Auto</A>"
			t += "[round(cell.percent())]% ([chargetext]) ([modetext])"
		else
			t += "<FONT COLOR=red>Not connected.</FONT>"
		t += "</td></tr>"
		t += "<tr><td><BR>Power channels</td><td></td></tr>"
		t += "<tr><td>Equipment<BR></td><td>[add_lspace(round(lastused_equip*factor), 6)] W/s : <B>[channelmodetext(equipment, equipment_percent)]<BR><A href='?src=\ref[src];percent=-100;chan=eqp'>AlwaysOn</A> <A href='?src=\ref[src];percent=-5;chan=eqp'>---</A> <A href='?src=\ref[src];percent=5;chan=eqp'>+++</A> <A href='?src=\ref[src];percent=100;chan=eqp'>AlwaysOff</A> <A href='?src=\ref[src];percent=default;chan=eqp'>Default</A></td></tr>"
		t += "<tr><td>Lighting<BR></td><td>[add_lspace(round(lastused_light*factor), 6)] W/s : <B>[channelmodetext(lighting, lighting_percent)]<BR><A href='?src=\ref[src];percent=-100;chan=lgt'>AlwaysOn</A> <A href='?src=\ref[src];percent=-5;chan=lgt'>---</A> <A href='?src=\ref[src];percent=5;chan=lgt'>+++</A> <A href='?src=\ref[src];percent=100;chan=lgt'>AlwaysOff</A> <A href='?src=\ref[src];percent=default;chan=lgt'>Default</A></td></tr>"
		t += "<tr><td>Environmental<BR></td><td>[add_lspace(round(lastused_environ*factor), 6)] W/s : <B>[channelmodetext(environ, environ_percent)]<BR><A href='?src=\ref[src];percent=-100;chan=env'>AlwaysOn</A> <A href='?src=\ref[src];percent=-5;chan=env'>---</A> <A href='?src=\ref[src];percent=5;chan=env'>+++</A> <A href='?src=\ref[src];percent=100;chan=env'>AlwaysOff</A> <A href='?src=\ref[src];percent=default;chan=env'>Default</A></td></tr>"
		t += "<tr><td>Total Load</td><td>[round((lastused_light + lastused_equip + lastused_environ)*factor)] W/s</td></tr>"
		t += "<tr><td>Cover Lock</td><td>[coverlocked ? "<B><A href='?src=\ref[src];lock=1'>Engaged</A></B>" : "<B><A href='?src=\ref[src];lock=1'>Disengaged</A></B>"]</td></tr>"
		t += "</table>"
...
//				world << "ticker says its malf"
				if (!src.malfai)
					...
					if(!src.occupant)
						...
						t += "<I>Core Processes Uploaded</I><BR><BR>"


	if(wiresexposed && !(get_dist(src, user) > 1)) //AIs cant see wires
	//var/t1 = text("<html><head><title>[area.name] APC wires</title></head><body><B>Access Panel</B><br>\n")
		...
		for(var/wiredesc in apcwirelist)
			...
			t += "[wiredesc] wire: "
			if(!is_uncut)
				t += "<a href='?src=\ref[src];apcwires=[apcwirelist[wiredesc]]'>Mend</a>"
			else
				t += "<a href='?src=\ref[src];apcwires=[apcwirelist[wiredesc]]'>Cut</a> "
				t += "<a href='?src=\ref[src];pulse=[apcwirelist[wiredesc]]'>Pulse</a> "
			t += "<br>"
		t += "</PRE>"
	...
	t += "</TT></body></html>"
	user << browse(t, "window=apc;size=450x400")
	onclose(user, "apc")
	return
...
/obj/machinery/power/apc/proc/update()
	...
	if(operating && !shorted && cell)
		var/percentage = cell.percent()
		if(percentage <= lighting_percent)
			lighting = 0
		if(percentage <= equipment_percent)
			equipment = 0
		if(percentage <= environ_percent)
			environ = 0
		if(percentage > 75)
			clear_alarm = 1

		power_light = lighting
		power_equip = equipment
	...
	var/update = 0
	if(area.power_light != power_light)
		area.power_light = power_light
		update = 1
	if(area.power_equip != power_equip)
		area.power_equip = power_equip
		update = 1
	if(area.power_environ != power_environ)
		area.power_environ = power_environ
		update = 1
	if(update)
		area.power_change()
		update_icon()
...
/obj/machinery/power/apc/Topic(href, href_list, var/usingUI = 1)
	...
	if (href_list["apcwires"])
		...
		var/val = text2num(href_list["percent"])
		switch(href_list["chan"])
			if("eqp")
				if(val == null)	equipment_percent = initial(equipment_percent)
				else			equipment_percent = Clamp(round(equipment_percent + val), 0, 100)
			if("lgt")
				if(val == null)	lighting_percent = initial(lighting_percent)
				else			lighting_percent = Clamp(round(lighting_percent + val), 0, 100)
			if("env")
				if(val == null)	environ_percent = initial(environ_percent)
				else			environ_percent = Clamp(round(environ_percent + val), 0, 100)
			else
				return
		update()
...
/obj/machinery/power/apc/proc/malfoccupy(var/mob/living/silicon/ai/malf)
	...
	src.occupant.oxy_damage = malf.oxy_damage
	if(!findtext(src.occupant.name,"APC Copy"))
		src.occupant.name = "[malf.name] APC Copy"
	if(!findtext(src.occupant.real_name,"APC Copy"))
		src.occupant.real_name = "[malf.real_name] APC Copy"
	if(malf.parent)
	...
	src.occupant.sight |= SEE_TURFS
	src.occupant.sight |= SEE_MOBS
	src.occupant.sight |= SEE_OBJS
	src.occupant.see_in_dark = 8
	src.occupant.see_invisible = SEE_INVISIBLE_LEVEL_TWO
	return
...
/obj/machinery/power/apc/proc/malfvacate(var/forced)
	...
	if(src.occupant.parent && src.occupant.parent.stat != 2)
		...
		src.occupant.parent.oxy_damage = src.occupant.oxy_damage
...
/obj/machinery/power/apc/load()
	if(terminal)
		return terminal.load()
	return 0

/obj/machinery/power/apc/proc/distribute_power(seconds)
	...
	var/net_surplus = 0
	if(terminal && terminal.powernet)
		net_supply = terminal.powernet.avail
		net_demand = terminal.powernet.load
		net_surplus = net_supply - net_demand

	if(net_supply <= 0)
		main_status = 0
	else if(net_surplus < 0)
		main_status = 1
		net_surplus = 0
	else
		main_status = 2
	...
	if(cell && !shorted)
		var/local_demand = lastused_total

		if(operating)
			var/cell_can_provide = cell.charge/CELLRATE
			var/contribution_from_cell = min(cell_can_provide, lastused_total)
			var/contribution_from_net = 0

			local_demand -= contribution_from_cell

			if(chargemode && net_surplus > 0)
				contribution_from_net = min(net_surplus, local_demand)
				local_demand -= contribution_from_net
				if(local_demand <= 0)
					net_surplus -= contribution_from_net
					terminal.powernet.load += contribution_from_net

			if(local_demand <= 0)
				cell.charge -= contribution_from_cell*CELLRATE
		...
		if(chargemode && net_surplus > 0)
			var/contribution_from_net = min(net_surplus, (cell.maxcharge - cell.charge)/CELLRATE)
			cell.charge += contribution_from_net*CELLRATE
...
/obj/machinery/power/apc/emp_act(severity)
	if(cell)
		...
		cell.charge = round(cell.charge/2)

	chargemode = rand(0,1)
	operating = rand(0,1)
	equipment_percent = rand(0,100)
	environ_percent = rand(0,100)
	lighting_percent = rand(0,100)
