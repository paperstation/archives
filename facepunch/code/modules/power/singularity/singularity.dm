...
/obj/machinery/singularity
	...
	proc/eat()
		set background = 1
		++SSpower.defer_powernet_rebuild
	...
	proc/consume(var/atom/A)
		...
		if (istype(A,/mob/living))//Mobs get gibbed
			...
			if(istype(A,/mob/living/carbon/human))
				...
				if(H.mind)
					if(H.mind.assigned_role == "Station Engineer")
						gain = 100
						H.unlock_achievement("At least you were good for something")
					else if(H.mind.assigned_role == "Chief Engineer")
						gain = 300
						H.unlock_achievement("At least you were good for something")
	...
	proc/check_turfs_in(var/direction = 0, var/step = 0)
		if(!direction)
			return 0
		var/steps = 0
		if(!step)
	...
	proc/toxmob()
		...
		for(var/mob/living/M in view(toxrange, src.loc))
			M.deal_damage(rand(radiationmin,radiation), IRRADIATE, IRRADIATE)
		return
	...
	proc/mezzer()
		for(var/mob/living/carbon/M in oviewers(8, src))
			...
			M.deal_damage(3, WEAKEN)
			visible_message("\red <B>[M] stares blankly at The [src]!</B>")
		return
