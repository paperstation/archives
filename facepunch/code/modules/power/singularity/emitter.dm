...
/obj/machinery/emitter
		...
		if(state == 2)
			if(!src.locked)
				if(src.active==1)
					...
					src.shot_number = 0
					src.fire_delay = 100
					src.use_power = USE_POWER_ACTIVE
	...
	process()
		...
		if(((src.last_shot + src.fire_delay) <= world.time) && (src.active == 1))
			...
			var/far_dist = 0
			for(var/mob/M in hearers(25, source))
				...
				if(M_turf && M_turf.z == source.z)
					...
					M.playsound_local(source, 'sound/weapons/emitter.ogg', far_volume, 1, 1, falloff = 5)
