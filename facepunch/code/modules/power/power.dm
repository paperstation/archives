...
/proc/makepowernets()
	for(var/datum/powernet/PN in SSpower.powernets)
		for(var/obj/structure/cable/PC in PN.cables)
			PC.powernet = null
		for(var/obj/machinery/power/M in PN.nodes)
			M.powernet = null
	SSpower.powernets.len = 0

	for(var/obj/structure/cable/PC in cable_list)
		if(!PC.powernet)
			PC.powernet = new()
			SSpower.powernets += PC.powernet
//			if(Debug)	world.log << "Starting mpn at [PC.x],[PC.y] ([PC.d1]/[PC.d2])"
			powernet_nextlink(PC,PC.powernet)

//	if(Debug) world.log << "[SSpower.powernets.len] powernets found"
...
/proc/powernet_nextlink(var/obj/O, var/datum/powernet/PN)
	...
	while(1)//Fuck
		...
		else if(istype(O,/obj/machinery/power) && O.anchored)
...
/datum/powernet/proc/reset()
	...
	//return any power previously provided by the smes, if it wasn't needed.
	if(netexcess > 100)
...
/proc/merge_powernets(var/datum/powernet/net1, var/datum/powernet/net2)
	...
	for(var/obj/machinery/power/Node in net2.nodes)		//merge net2 into net1
		Node.powernet = net1
