/mob/living/carbon/human/say(var/message)
	...
	if (disabilities & TOURETTES)
		if(copytext(message, 1, 2) != "*")
			if(prob(15))
				message = replacetext(message, "captain", "FAGGOT")
				message = replacetext(message, "assistant", "ASSHOLE")
				message = replacetext(message, "the", "the FUCK")
				message = replacetext(message, "up", "FUCK UP")
				message = replacetext(message, "help", "FUCK")
				message = replacetext(message, "hello", "PISS OFF")
				message = replacetext(message, "admin", "FUCKER")
				message = replacetext(message, "gloves", "SHIT")
				message = replacetext(message, "beepsky", "MOTHERFUCKER")
				message = replacetext(message, "cmo", "CUNT")
				message = replacetext(message, "the singularity", "THE ANUS")
				message = replacetext(message, "singulo", "ANUS")
				message = replacetext(message, "head", "DICK")
				message = replacetext(message, "masturbating", "FUCKING")
				message = replacetext(message, "Woody", "[real_name]")
	if(wear_mask)
		if(istype(wear_mask, /obj/item/clothing/mask/gas/voice/space_ninja) && wear_mask:voice == "Unknown")
			...
			if(copytext(message, 1, 2) != "*")
				...
				message = replacetext(message, "medical", "médico")
				message = replacetext(message, "engineering", "ingeniería")
				message = replacetext(message, "security", "seguridad")
				message = replacetext(message, "man", "hombre")
				message = replacetext(message, "women", "senorita")
				message = replacetext(message, "errorage", "shitty coderica")
