...
/mob/living/carbon/human/UpdateDamageIcon(var/update_icons=1)
	...
	standing.overlays	+= "head_[min(3,round((brute_head/100)*3, 1))]0"
	lying.overlays		+= "head2_[min(3,round((brute_head/100)*3, 1))]0"
	standing.overlays	+= "head_0[min(3,round((fire_head/100)*3, 1))]"
	lying.overlays		+= "head2_0[min(3,round((fire_head/100)*3, 1))]"

	standing.overlays	+= "chest_[min(3,round((brute_chest/100)*3, 1))]0"
	lying.overlays		+= "chest2_[min(3,round((brute_chest/100)*3, 1))]0"
	standing.overlays	+= "chest_0[min(3,round((fire_chest/100)*3, 1))]"
	lying.overlays		+= "chest2_0[min(3,round((fire_chest/100)*3, 1))]"

	standing.overlays	+= "arms_[min(3,round((brute_arms/50)*3, 1))]0"
	lying.overlays		+= "arms2_[min(3,round((brute_arms/50)*3, 1))]0"
	standing.overlays	+= "arms_0[min(3,round((fire_arms/50)*3, 1))]"
	lying.overlays		+= "arms2_0[min(3,round((fire_arms/50)*3, 1))]"

	standing.overlays	+= "legs_[min(3,round((brute_legs/50)*3, 1))]0"
	lying.overlays		+= "legs2_[min(3,round((brute_legs/50)*3, 1))]0"
	standing.overlays	+= "legs_0[min(3,round((fire_legs/50)*3, 1))]"
	lying.overlays		+= "legs2_0[min(3,round((fire_legs/50)*3, 1))]"
...
/mob/living/carbon/human/proc/update_body(var/update_icons=1)
	if(stand_icon)	stand_icon = null
	if(lying_icon)	lying_icon = null
		...
		switch(dna.mutantrace)
			...
			if("plant")
				if(stat == DEAD)	//TODO
					...
					overlays_lying[MUTANTRACE_LAYER]	= image("icon" = 'icons/effects/genetics.dmi', "icon_state" = "[dna.mutantrace][fat]_[gender]_l")
					overlays_standing[MUTANTRACE_LAYER]	= image("icon" = 'icons/effects/genetics.dmi', "icon_state" = "[dna.mutantrace][fat]_[gender]_s")
		//			var/obj/item/organ/heart/B = locate() in src.internal_organs
		//			B.tomato()
			if("orange")
				color = "yellow"
			else
...
/mob/living/carbon/human/update_inv_w_uniform(var/update_icons=1)
	if(w_uniform && istype(w_uniform, /obj/item/clothing/under) )
		...
		if(w_uniform:hastie)	//WE CHECKED THE TYPE ABOVE. THIS REALLY SHOULD BE FINE.
			var/tie_color = w_uniform:hastie.variant
