...
/datum/gas_mixture
	...
	proc/return_pressure()
		if(volume <= 0)
			return 0
		return max(total_moles()*R_IDEAL_GAS_EQUATION*temperature/volume,0)
	...
	proc/check_tile_graphic()
		...
		if(toxins > MOLES_PLASMA_VISIBLE)
			...
			var/datum/gas/agent_b = locate(/datum/gas/oxygen_agent_b) in trace_gases
			if(sleeping_agent && (sleeping_agent.moles > 1))
				graphic = "sleeping_agent"
			else if(agent_b && (agent_b.moles > 1))
				graphic = "agentb"
			else
				graphic = null
