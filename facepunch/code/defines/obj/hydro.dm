...
/obj/item/seeds/alienplantseed
	name = "pack of Alien Plant seeds"
	desc = "Maybe you should use robust harvest on it when its done?"
	icon_state = "seed-replicapod"
	species = "replicapod"
	plantname = "Replica Pod"
	product = /mob/living/simple_animal/alienplant/
	lifespan = 25
	endurance = 25
	maturation = 1
	production = 1
...
/obj/item/seeds/toolbox
	name = "pack of toolbox seeds"
	desc = "Sounds like your kind of seeds."
	icon_state = "seed-replicapod"
	species = "replicapod"
	plantname = "TOOLBOX"
	product = /obj/item/weapon/storage/toolbox/mechanical
	lifespan = 25
	endurance = 25
	maturation = 1
	production = 1
...
/obj/item/weapon/storage/bag/nutrientstor
	name = "NutriStor"
	desc = "You can hold about 10 nutrient bottles of any type!."
	icon = 'icons/obj/device.dmi'
	icon_state = "nutri"
	item_state = "nutri"
	...
	max_w_class = 3
	can_hold = list("/obj/item/nutrient")


/*
/obj/item/weapon/nutrientstor
	name = "NutriStor"
	icon = 'icons/obj/device.dmi'
	icon_state = "nutri"
	item_state = "nutri"
...
/obj/item/weapon/nutrientstor/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W,/obj/item/nutrient/ez))
		if(uses <= 4)
			if(mixed == 0)
				user.drop_item()
				del(W)
				user << "You insert the nutrient into the [name]. It has [uses] worth of nutrients in it."
				uses += 1

/obj/item/weapon/nutrientstor/attack(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W,/obj/item/nutrient/ez))
		if(uses <= 4)
			if(mixed == 0)
				user.drop_item()
