...
/obj/item/weapon/mousetrap/examine()
	...
	if(nade)
		usr << "\red It looks like someone's rigged a grenade to it."
...
/obj/item/weapon/mousetrap/attack_self(mob/user as mob)
	if(!armed)
		...
		if((user.brainloss >= 60 || user.mutations & CLOWN) && prob(50))
			...
			return
		if(nade)
			user << "\blue You remove the grenade from the mousetrap."
...
/obj/item/weapon/mousetrap/HasEntered(AM as mob|obj)
	if((ishuman(AM)) && (armed) && !istype(AM, /obj/item/weapon/mousetrap))
		...
		if(H.m_intent == "run")
			...
			for(var/mob/O in viewers(H, null))
				if(O == H)
					continue
				O.show_message(text("\red <B>[H.name] steps on the mousetrap.</B>"), 1)
	if((armed) && !istype(AM, /obj/item/weapon/mousetrap))
		var/mob/living/carbon/H = AM
		//H.take_organ_damage(101)
		src.triggered(H)
		H << "\red <B>You accidentally step on the mousetrap!</B>"
		for(var/mob/O in viewers(H, null))
			if(O == H)
...
/obj/item/weapon/mousetrap/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if (istype(W, /obj/item/weapon/chem_grenade) && !nade)
		user.u_equip(W)
		user.client.screen -= W
		W.loc = src
		nade = W
		user << "\blue You arm the mousetrap with [W]!</B>"
		icon_state = "mousetrapnade"
		if(!armed)
			armed = 1

/obj/item/weapon/astronautfoodpacket/update_icon()
	src.icon_state = text("astronautfood[]", src.foodcount)
	src.desc = text("There are [] rations\s left!", src.foodcount)
	return

/obj/item/weapon/astronautfoodpacket/attack_hand(mob/user as mob)
	if(user.r_hand == src || user.l_hand == src)
		if(src.foodcount == 0)
			user << "\red This packet is empty."
			return
		else
			src.foodcount--
			var/obj/item/weapon/reagent_containers/food/snacks/rationcube/R = new /obj/item/weapon/reagent_containers/food/snacks/rationcube(user)
			if(user.hand)
				user.l_hand = R
			else
				user.r_hand = R
